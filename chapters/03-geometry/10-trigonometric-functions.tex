...
\end{tikzpicture}
\captionof{figure}{锐角三角函数定义（以直角三角形中 $\angle C = \alpha$ 为例）}
...
\begin{tikzpicture}[every node/.style={font=\small}, scale=0.9]
  ...
  \end{scope}
\end{tikzpicture}
\captionof{figure}{特殊角三角函数的精确值（$30°$-$60°$-$90°$ 和 $45°$-$45°$-$90°$ 三角形）}
...
\end{tikzpicture}
\captionof{figure}{仰角与俯角：从水平视线分别向上（仰角）和向下（俯角）量起}
...
\end{tikzpicture}
\captionof{figure}{俯角例题：$\tan\alpha = h/d$，已知两量可求第三量}
