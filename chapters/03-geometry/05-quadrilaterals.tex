...
\end{tikzpicture}
\caption{平行四边形 $ABCD$：对角线 $AC$ 和 $BD$ 互相平分于点 $O$}
...
\end{tikzpicture}
\caption{梯形：$AB \parallel DC$，$AB$ 为下底，$DC$ 为上底，$AD$、$BC$ 为两腰}
\label{fig:trapezoid}
\end{figure}
...
\end{tikzpicture}
\caption{梯形面积 $S = \dfrac{(a+b) \cdot h}{2}$，$h$ 为两底之间的距离（高）}
