...
\begin{tikzpicture}[every node/.style={font=\small}]
  ...
  \coordinate (D) at (0.7,2);
  \draw[main line] (A)--(B)--(C)--(D)--cycle;
  ...
  \draw[aux line] (A)--(C);
  \draw[aux line] (B)--(D);
  % Midpoint (intersection of diagonals)
  \coordinate (O) at ($(A)!0.5!(C)$);
  \fill (O) circle (2pt) node[above right, font=\scriptsize] {$O$};
  % Parallel tick marks on AB || DC
  \draw ($(A)!0.5!(B)$)++(90:0.1)--++(0,-0.2);
  \draw ($(A)!0.5!(B)$)++(90:0.1)++(0.18,0)--++(0,-0.2);
  \draw ($(D)!0.5!(C)$)++(90:0.1)--++(0,-0.2);
  \draw ($(D)!0.5!(C)$)++(90:0.1)++(0.18,0)--++(0,-0.2);
  % Single tick on AD || BC
  \draw ($(A)!0.5!(D)$)++(0:0.12)--++(-0.24,0);
  \draw ($(B)!0.5!(C)$)++(0:0.12)--++(-0.24,0);
\end{tikzpicture}
\captionof{figure}{平行四边形 $ABCD$：对角线 $AC$ 和 $BD$ 互相平分于点 $O$}
...
\begin{tikzpicture}[every node/.style={font=\small}]
  ...
  \draw ($(A)!0.5!(B)$)++(90:0.1)--++(0,-0.2);
  \draw ($(D)!0.5!(C)$)++(90:0.1)--++(0,-0.2);
  \node[below] at ($(A)!0.5!(B)$) {$AB$（下底）};
  \node[above] at ($(D)!0.5!(C)$) {$DC$（上底）};
\end{tikzpicture}
\captionof{figure}{梯形：$AB \parallel DC$，$AB$ 为下底，$DC$ 为上底，$AD$、$BC$ 为两腰}
\label{fig:trapezoid}
\end{center}
...
\begin{tikzpicture}[every node/.style={font=\small}]
  ...
  \draw[aux line] (D)--(H);
  \draw (1.02,0)--(1.02,0.22)--(0.8,0.22);
  \fill (H) circle (2pt) node[below] {$H$};
  % Height arrow
  \draw[<->, >=Stealth, thin] (-0.35,0) -- (-0.35,2) node[midway,left] {$h$};
  % Upper base label
  \draw[<->, >=Stealth, thin] (0.8,2.4) -- (3.5,2.4) node[midway,above] {$b$（上底）};
  % Lower base label
  \draw[<->, >=Stealth, thin] (0,-0.4) -- (4.5,-0.4) node[midway,below] {$a$（下底）};
\end{tikzpicture}
\captionof{figure}{梯形面积 $S = \dfrac{(a+b) \cdot h}{2}$，$h$ 为两底之间的距离（高）}
\label{fig:trapezoid-height}
\end{center}
...
\begin{tikzpicture}[
  every node/.style={draw, rounded corners, align=center, font=\small,
                     minimum width=2.4cm, minimum height=0.7cm, fill=white},
  level 1/.style={sibling distance=4cm, level distance=1.5cm},
  edge from parent/.style={draw, -{Stealth[length=5pt]}},
]
  \node {四边形} [grow=down]
    child { node {平行四边形\\$AB\parallel DC,\ AD\parallel BC$}
      child { node {矩形\\（四角直角）} }
      child { node {菱形\\（四边相等）}
        child { node {正方形} }
      }
    ...
    child { node {梯形\\（一组平行）} };
\end{tikzpicture}
\captionof{figure}{四边形家族关系（从上到下为特殊化关系）}
\label{fig:quadrilateral-family}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
  every node/.style={draw, rounded corners, fill=orange!10, font=\small, inner sep=3pt},
  arr/.style={->, thick, gray!60},
  node distance=7mm and 14mm,
]
\node (quad) {四边形};
\node[below left=8mm and 14mm of quad] (trap) {梯形};
\node[below right=8mm and 14mm of quad] (para) {平行四边形};
\node[below=8mm of trap] (itrap) {等腰梯形};
\node[below left=8mm and 8mm of para] (rect) {矩形};
\node[below right=8mm and 8mm of para] (rhom) {菱形};
\node[below right=8mm and 0mm of rect] (sq) {正方形};
\draw[arr] (quad) -- (trap);
\draw[arr] (quad) -- (para);
\draw[arr] (trap) -- (itrap);
\draw[arr] (para) -- (rect);
