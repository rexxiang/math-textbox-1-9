...
\end{tikzpicture}
\caption{三角形内角和定理证明：过 $A$ 作 $BC$ 的平行线}
...
\end{tikzpicture}
\caption{SSA 反例：相同的两边和非夹角无法唯一确定三角形}
...
\begin{tikzpicture}[every node/.style={font=\small}, scale=0.9]
  ...
  \node at (4.7,1) {$\cong$};
\end{tikzpicture}
\caption{SAS：$AB=DE$，$\angle A=\angle D$，$AC=DF$ $\Rightarrow$ $\triangle ABC \cong \triangle DEF$}
...
\begin{tikzpicture}[every node/.style={font=\small}, scale=0.9]
  ...
  \node at (4.7,1.2) {$\cong$};
\end{tikzpicture}
\caption{AAS：$\angle A=\angle D$，$\angle B=\angle E$，$BC=EF$ $\Rightarrow$ $\triangle ABC \cong \triangle DEF$}
...
\end{tikzpicture}
\caption{等腰三角形：$AB=AC$，底角 $\angle B = \angle C$（等腰三角形的底角相等）}
...
\end{tikzpicture}
\caption{等腰三角形中，顶角的角平分线、底边中线、底边上的高三线合一}
...
\end{tikzpicture}
\caption{相似三角形 $\triangle ABC \sim \triangle DEF$，相似比 $2:1$，$6:3=8:4=10:5$}
...
\end{tikzpicture}
\caption{三角形中位线定理：$M$、$N$ 分别是 $AB$、$AC$ 中点，则 $MN \parallel BC$ 且 $MN = \frac{1}{2}BC$}
