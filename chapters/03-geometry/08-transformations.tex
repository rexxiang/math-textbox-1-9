...
\end{tikzpicture}
\caption{轴对称：$\triangle A'B'C'$ 是 $\triangle ABC$ 关于直线 $l$ 的对称图形}
...
\end{tikzpicture}
\caption{最短路径问题：利用轴对称，过河取水的最短路径为直线 $A'B$}
...
\begin{tikzpicture}[every node/.style={font=\small}]
  ...
  \draw[aux line] (B)--(Bp);
\end{tikzpicture}
\caption{中心对称：$\triangle A'B'C'$ 是 $\triangle ABC$ 关于点 $O$ 的中心对称图形}
...
\end{tikzpicture}
\caption{平移变换：$\triangle A'B'C'$ 是 $\triangle ABC$ 沿向量 $\vec{v}$ 平移的结果}
...
\end{tikzpicture}
\caption{旋转变换：$\triangle A'B'C'$ 是 $\triangle ABC$ 绕点 $O$ 逆时针旋转 $90°$ 的结果}
\label{fig:rotation}
\end{figure}
