...
**注意**：平均数不一定等于数据中的某个实际值。例如上面的例子中，平均数是 8，而实际数据中只有第 1 组恰好是 8。

### 关键总结（Key Takeaways）

- 平均数 $= \frac{\text{数据总和}}{\text{数据个数}}$ 。
...
但这没有考虑到各项成绩的**重要程度不同**——期末考试占比最大，应该对总评的影响也更大。

### 概念建立（Build Understanding）

当各数据的**重要程度不同**时，需要计算**加权平均数**。每个数据乘以它的**权重**（即重要程度所占的比例），然后求和。
...
**问题出在哪里？** 50000 是一个**极端值**（远大于其他数据），它把平均数"拉"高了。我们需要一种不受极端值影响的度量。

### 概念建立（Build Understanding）

**中位数**是将数据**从小到大排列**后，处于**正中间位置**的那个数。

求法：
1. 将 $n$ 个数据从小到大排列。
2. 如果 $n$ 是**奇数**，中位数是第 $\frac{n+1}{2}$ 个数据。
3. 如果 $n$ 是**偶数**，中位数是第 $\frac{n}{2}$ 个与第 $\frac{n}{2}+1$ 个数据的平均值。

**回到上面的例子**：数据已按从小到大排列，共 7 个（奇数），中位数是第 $\frac{7+1}{2} = 4$ 个数据，即 $4200$ 元。

$4200$ 元比 $10600$ 元更能代表大多数员工的薪资水平。

**偶数个数据的例子**：如果有 8 个数据 $2, 5, 7, 8, 10, 12, 15, 20$ ，则中位数为第 4 个和第 5 个数据的平均值：
...
**例 2.** 10 名同学的数学考试成绩为：72, 85, 90, 65, 78, 92, 88, 76, 95, 82。求这组数据的中位数。
...
**特别说明**：
- 一组数据可以有**多个众数**（如果有两个或更多值出现次数相同且最多）。
- 也可能**没有众数**（如果所有值出现次数都一样）。
...
算一算：
- 平均数 $= \frac{10 \times 3 + 20 \times 2 + 30 + 50 \times 2 + 200}{9} = \frac{30 + 40 + 30 + 100 + 200}{9} = \frac{400}{9} \approx 44.4$ 元
- 中位数 $= 20$ 元（第 5 个数据）
- 众数 $= 10$ 元（出现 3 次，最多）
...
- 当数据分布比较均匀、没有极端值时，**平均数**最常用。
- 当数据中有明显的极端值（如薪资数据、房价数据）时，**中位数**更合适。
- 当关心"出现最多的是哪个"时（如衣服号码、投票结果），用**众数**。
- 在许多实际问题中，可以同时报告多个统计量，给出更完整的信息。
...
- 甲的成绩集中在 $7 \sim 10$ 之间，比较稳定。
- 乙的成绩从 $4$ 到 $10$ 波动很大，虽然高分更多，但也有很低的分数。

如果看**稳定性**，甲更好（数据波动小）。如果看**高分能力**，乙的众数 $10$ 说明他更常打出满分。这告诉我们：**只看一种统计量可能不够全面**，有时需要综合考虑。（在 §4.3 中我们将学习用"方差"来精确衡量数据的波动程度。）
...
**1.** 某超市一周内某款饮料每天的销量（瓶）为： $45, 52, 38, 52, 60, 52, 41$ 。
（1）求这组数据的平均数、中位数和众数。
（2）如果超市想知道每天应准备多少瓶该饮料，你建议用哪个统计量？

**2.** 某班 8 名同学 1 分钟仰卧起坐的个数为： $25, 30, 28, 32, 45, 29, 31, 28$ 。
（1）求平均数和中位数。
（2）如果有一名同学的成绩改为 $80$ 个（特别突出），平均数和中位数各变为多少？
（3）哪个统计量受极端值的影响更小？

**3.** 一次面试中，综合成绩由笔试（权重 $0.3$ ）、面试表现（权重 $0.5$ ）和简历评分（权重 $0.2$ ）三部分组成。张三的三项得分分别为 88 分、92 分、75 分，李四的三项得分分别为 95 分、80 分、90 分。谁的综合成绩更高？
...
（2）建议用**众数** $52$ 或**中位数** $52$ 。因为超市关心的是"大多数时候卖多少"，而平均数受到某天低销量（ $38$ ）的影响偏低，可能导致准备不足。
