...
如果两个图形关于某条直线对称，则这两个图形叫做**轴对称图形**（成轴对称）。

![轴对称示意图](../assets/03-geometry/axis-symmetry.svg)
...
如果两个图形关于某个点对称，就说这两个图形**成中心对称**。

![中心对称示意图](../assets/03-geometry/central-symmetry.svg)
...
**常见中心对称图形**：
- 平行四边形（对角线交点为对称中心）
- 矩形、菱形、正方形（均以对角线交点为对称中心）
- 圆（圆心为对称中心）
- 正偶数边形（正四边形、正六边形等）

> 注意：等边三角形是轴对称图形，但**不是**中心对称图形。正三角形、正五边形等正奇数边形都不是中心对称图形。
...
4. 求点 $M(3, -1)$ 关于点 $N(1, 2)$ 的对称点。
5. 下列图形中，既是轴对称又是中心对称的有哪些？(a) 矩形 (b) 等腰三角形 (c) 菱形 (d) 正五边形 (e) 圆。
...
![平移示意图](../assets/03-geometry/translation.svg)
...
![旋转示意图](../assets/03-geometry/rotation.svg)

**旋转的性质**：
1. 旋转不改变图形的形状和大小。
2. 对应点到旋转中心的距离相等。
3. 对应点与旋转中心的连线所成的角等于旋转角。
4. 对应线段相等，对应角相等。
...
- 旋转不改变形状和大小。
- 正 $n$ 边形绕中心旋转 $\dfrac{360°}{n}$ 后与自身重合。
...
共同点：三种变换都是**等距变换**（保距变换），即变换前后对应点之间的距离不变，图形的形状和大小不变。
...
**例 3.** 分析下面的图案使用了哪些图形变换：一排等距排列的完全相同的三角形。

**解：**
这个图案使用了**平移**变换。将第一个三角形沿排列方向平移固定的距离，就得到第二个、第三个......每一个三角形都是前一个的平移像。
