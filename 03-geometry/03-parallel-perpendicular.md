...
当一条直线（称为**截线**）与两条直线相交时，会形成 $8$ 个角。这些角之间有特殊的名称和位置关系。
...
| 角对类型 | 位置特征 | 示例 |
|----------|----------|------|
| **同位角** | 在截线同侧，在两条被截线的同方位 | $\angle 1$ 与 $\angle 5$ |
| **内错角** | 在截线两侧，在两条被截线之间 | $\angle 3$ 与 $\angle 5$ |
| **同旁内角** | 在截线同侧，在两条被截线之间 | $\angle 3$ 与 $\angle 6$ |

**平行线的判定方法**（由角的关系判断两直线平行）：
...
> **性质与判定的区别**：
> - **判定**：角的关系 $\Rightarrow$ 平行（用来"证明平行"）
> - **性质**：平行 $\Rightarrow$ 角的关系（用来"求角的度数"）
>
> 两者方向相反，不可混淆。
...
> 注意和平行公理的对比：
> - 过直线**外**一点，有且只有一条直线与已知直线**平行**。
> - 过一点（直线上或直线外），有且只有一条直线与已知直线**垂直**。
...
站在河边想去对岸，怎样走最近？当然是走"垂直于河岸"的路线。这背后的原理就是**垂线段最短**。
