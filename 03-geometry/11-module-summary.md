...
特别注意：同一条弧上的**所有圆周角**都相等（不管顶点在弧上哪个位置），但圆心角只有一个。

### 3. 平行线的"判定"与"性质"方向搞反

- **判定**：角的关系 $\Rightarrow$ 平行（用来**证明**两直线平行）
- **性质**：平行 $\Rightarrow$ 角的关系（用来**求**角的度数）

错误示范："因为 $a \parallel b$，所以同位角相等，所以 $a \parallel b$。"（循环论证）

### 4. 等腰三角形忘记分类讨论

题目说"等腰三角形的一个角是 $x°$"，必须考虑这个角是**顶角还是底角**两种情况，除非条件明确排除某一种。

例如：等腰三角形一个角是 $120°$，这只能是顶角（两个底角不可能都是 $120°$）。但如果是 $50°$，则顶角和底角两种情况都有可能。

### 5. 勾股定理误用于非直角三角形

勾股定理 $a^2 + b^2 = c^2$ **只适用于直角三角形**，其中 $c$ 是斜边（直角的对边）。对于非直角三角形，这个等式不成立。

使用勾股定理前，必须先确认（或证明）三角形有一个直角。

### 6. 三角函数的边搞混

$\sin$、$\cos$、$\tan$ 的定义都是**相对于某个特定角**而言的。同一个直角三角形中，$\sin A \neq \sin B$（除非 $A = B = 45°$）。

常见错误：把对边和邻边搞反。记住：**对边是角"对面"的那条边，邻边是角"旁边"的直角边**。斜边永远是最长的那条（直角的对边）。
...
我们学习的几何（欧几里得几何）建立在五条公理之上，其中第五条——**平行公理**——说"经过直线外一点，有且只有一条直线与已知直线平行"。

数学家们曾经尝试证明这条公理可以从其他四条推导出来，但始终失败。19 世纪初，俄国数学家罗巴切夫斯基和匈牙利数学家鲍耶各自独立地发现：如果把平行公理替换为"经过直线外一点，有不止一条直线与已知直线不相交"，也能建立一套自洽的几何学。

这种几何学叫做**双曲几何**（罗氏几何）。在双曲几何中，三角形内角和小于 $180°$；在另一种叫做**椭圆几何**（黎曼几何）的体系中，三角形内角和大于 $180°$。
...
1852 年，英国数学家弗朗西斯·格思里提出猜想：**四种颜色就够了**。这个猜想听起来简单，却困扰了数学家一百多年。
...
把一条线段分成两段，使得长段与短段的比等于全段与长段的比，这个比值就是**黄金比例**：
