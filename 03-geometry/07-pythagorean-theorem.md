...
![勾股定理示意图](../assets/03-geometry/pythagorean-theorem.svg)
...
![赵爽弦图](../assets/03-geometry/zhao-shuang-proof.svg)
...
**另一种理解**：也可以将四个相同直角三角形拼成一个边长为 $(a + b)$ 的大正方形。
...
**例 1.** 一架 $5$ 米长的梯子斜靠在墙上，梯脚距墙 $3$ 米。求梯子顶端离地面的高度。
...
**例 4.** 在等腰三角形 $ABC$ 中，$AB = AC = 10$ cm，$BC = 12$ cm。求 $BC$ 边上的高 $AD$ 的长度。
...
> 能使 $a^2 + b^2 = c^2$ 成立的三个正整数 $(a, b, c)$ 叫做**勾股数**。常见的勾股数组有：
...
**例 2.** $\triangle ABC$ 中，$AB = 15$，$BC = 20$，$AC = 25$。判断 $\triangle ABC$ 的形状，并求面积。
...
**例 3.** 三角形的三边长为 $a = \sqrt{2}$、$b = \sqrt{3}$、$c = \sqrt{5}$。判断是否为直角三角形。
...
- 勾股定理的逆定理可用于判定直角三角形。
- 利用 $a^2 + b^2$ 与 $c^2$ 的比较，还能判断三角形是锐角还是钝角。
- 常见勾股数：$(3,4,5)$、$(5,12,13)$、$(8,15,17)$、$(7,24,25)$。

### 练一练（Practice）

5. 判断以下各组长度能否构成直角三角形：(a) $11, 60, 61$；(b) $1, 1, \sqrt{2}$；(c) $4, 5, 6$。
6. 一块三角形地，三边长分别为 $9$ m、$12$ m、$15$ m。这块地是什么形状？求面积。
7. 判断三边为 $\sqrt{3}, 2, \sqrt{7}$ 的三角形是什么类型的三角形。
8. 在 $\triangle ABC$ 中，$a = 2k$，$b = k^2 - 1$，$c = k^2 + 1$（$k > 1$）。证明 $\triangle ABC$ 是直角三角形。

---

## 参考答案

1. $c = \sqrt{5^2 + 12^2} = \sqrt{25 + 144} = \sqrt{169} = 13$ cm。

2. $b = \sqrt{17^2 - 8^2} = \sqrt{289 - 64} = \sqrt{225} = 15$ cm。

3. 折断处到地面 $3$ m，折断的部分为斜边。折断部分长 $= \sqrt{3^2 + 4^2} = 5$ m。树折断前的高度 $= 3 + 5 = 8$ m。

4. 设边长为 $a$，对角线 $= a\sqrt{2} = 8$，$a = \dfrac{8}{\sqrt{2}} = 4\sqrt{2} \approx 5.66$ cm。
...
6. $9^2 + 12^2 = 81 + 144 = 225 = 15^2$，是直角三角形。面积 $= \dfrac{1}{2} \times 9 \times 12 = 54$ m$^2$。

7. $(\sqrt{3})^2 + 2^2 = 3 + 4 = 7 = (\sqrt{7})^2$，是直角三角形。
