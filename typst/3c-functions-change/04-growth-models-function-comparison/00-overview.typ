...
#blueprint[
  *难度*：★★★★★ \
  *核心问题*：本分支到现在已经装好了线性、反比例、二次三类*代数*函数族。现实里还有另一种非常常见的节奏——*每加 $1$ 单位 $x$，$y$ 乘同一倍数*（乘法增长）——它短期看不出厉害，长期却*碾压*前面几族。本章回答两件事：（1）怎么识别加法 / 乘法两种增长？（2）拿到一张陌生表 / 一张陌生图，怎样在四族里挑出最合适的那个？*长期*它们又有怎样稳定的*级别排序*？ \
  ...
  *本章在全书的位置*：函数分支的*现象层顶点*。上一章装完代数层三族工具，本章补一个*现象标签*（乘法增长，不作为 blueprint 正式族）、升级成模型选择流程，并把四族的*长期等级*固定下来，作为往高中过渡的心智模型。 \
