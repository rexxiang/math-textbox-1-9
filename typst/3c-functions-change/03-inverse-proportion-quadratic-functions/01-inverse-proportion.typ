...
#discovery[
  *先用旧工具试试。* 分苹果：$x$ 人分 $12$ 个，每人拿 $y$ 个。$x$ 增大时 $y$ 减小，正比例肯定不行。也许是"$y$ 减去某个常数"？试 $y = C - x$：当 $x = 1$ 时 $y = 12$，得 $C = 13$；当 $x = 2$ 时预测 $13 - 2 = 11$——实际是 $6$，差了 $5$。

  试着推下去——

  再试 $y = C - 2x$：$x = 1$ 时 $C = 14$；$x = 3$ 时预测 $8$，实际 $4$，差 $4$；$x = 4$ 时预测 $6$，实际 $3$，差 $3$；$x = 6$ 时预测 $2$，实际 $2$——刚好。但不同数据点要求不同的 $C$ 或斜率，没有一个线性公式能同时拟合所有点。

  *这里卡住了。* 加法增长无法描述"$x$ 翻倍时 $y$ 减半"的节奏——数据里隐藏的规律是乘积 $x y = 12$ 恒定，不是差恒定，需要一种新的函数形式。
