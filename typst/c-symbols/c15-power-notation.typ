...
#discovery[
  在 #secref("c12-polynomial-multiply") 里，你已经见过 $x^2$ 这种"平方"的短写。现在把这个想法再往前推进：
...
#mastery[
  ...
  *应用*
  ...
  + 一个细菌每小时分裂一次（数量翻倍），初始只有 1 个。用幂的记号写出 8 小时后的细菌数量。如果改用普通乘法一个个写出来，你要写多少个"$times 2$"？

  *挑战*

  + 若 $x^m times x^2 = x^9$，求 $m$。
  + 解释：为什么 $a^3 div a^3$ 按定义等于 1，而按同底数幂相除规则等于 $a^0$？这两件事合在一起说明了什么？
  + *识别错误题*：下面四道计算，哪些是错误的？找出来并改正：
    - (a) $x^3 times x^4 = x^7$
    - (b) $x^3 times y^4 = (x y)^7$
    - (c) $x^6 div x^2 = x^3$
    - (d) $(x^3)^2 = x^6$
  ...
  *应用*
  ...
  + $x^3$
  + $a^3 times a^2 = a^5$
  + 8 小时后有 $2^8 = 256$ 个细菌。用普通乘法要写 7 个"$times 2$"（即 $1 times 2 times 2 times dots.c times 2$，共 8 个 2、7 个乘号）。幂的记号把这一长串压缩成了 $2^8$，简洁得多。

  *挑战*
  + $m + 2 = 9$，所以 $m = 7$
  + 按定义，$a^3 div a^3 = 1$（同一个数除以自己）；按规则，$a^3 div a^3 = a^(3-3) = a^0$。两件事合在一起，就强烈暗示 $a^0 = 1$——这个结论在下一节会正式推导。
  + 错误的是 (b) 和 (c)。
    - (a) ✓ 同底数幂相乘，指数相加：$x^3 times x^4 = x^7$
    - (b) ✗ $x^3 times y^4$ 底数不同，不能把指数相加；正确的写法就是 $x^3 y^4$，无法化简为 $(x y)^?$
    - (c) ✗ 同底数幂相除，指数相减：$x^6 div x^2 = x^(6-2) = x^4$，不是 $x^3$
    - (d) ✓ 幂的乘方，指数相乘：$(x^3)^2 = x^(3 times 2) = x^6$
