...
#blueprint[
  ...
  *小数*（用负指数）：细胞直径约 $0.00001$ m $= 1 times 10^(-5)$ m。
  ...
  规则：小数点移动 $n$ 位化为 $a$ 的形式：
  - 原数 $>= 10$：小数点左移 $n$ 位，指数为正 $n$
  - 原数 $< 1$：小数点右移 $n$ 位，指数为负 $-n$
]
...
#blueprint[
  回顾：开平方是平方的逆运算（已知 $a^2$ 求 $a$），开立方是立方的逆运算（已知 $a^3$ 求 $a$）。

  如果指数运算法则对分数指数也成立，那么：
  $ (a^(1\/2))^2 = a^(1\/2 times 2) = a^1 = a $

  所以 $a^(1\/2)$ 恰好就是 $sqrt(a)$。类似地 $a^(1\/3) = root(3, a)$。一般地：
