...
#discovery[
  ...
  *第二步*：中心对称——旋转 $180 degree$。
  ...
  例如：一般的平行四边形是中心对称图形（对称中心是对角线的交点），但不是轴对称图形。（注意：矩形、菱形、正方形虽然也是平行四边形，但它们既是中心对称图形也是轴对称图形。）
...
#blueprint[
  *位似*是一种特殊的相似变换：从一个定点（*位似中心*）出发，所有对应点到中心的距离按同一比例缩放。

  设位似中心为 $O$，位似比为 $k$（$k > 0$，$k != 1$）：
  - $k > 1$：放大
  - $0 < k < 1$：缩小
  - 对应点、位似中心三点共线
  - 对应线段平行（或在同一直线上）
  ...
  *坐标表示*（以原点为位似中心）：
  ...
  位似是相似的特例：位似变换后的图形与原图形*相似*，相似比等于位似比 $k$。
]
