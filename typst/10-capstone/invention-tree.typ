...
- *代数深化*：在 #secref("6.1") 到 #secref("6.7") 里，你不断把“关系写出来再操作”的能力做得更稳，从模板走到求解，从求一个数走到限制一整个范围。
- *几何深化*：在 #secref("7.1") 到 #secref("7.8") 里，你把“看起来对”推进成“必须如此”，又把测量问题接到相似、勾股与三角比上；这里的三角比在 #secref("7.6")，属于几何测量链的一部分。
- *函数模型*：在 #secref("8.1") 到 #secref("8.4") 里，你不断比较“怎样变化”，把比例、坐标、公式和图象收进同一个模型视角。
- *数据统计*：在 #secref("9.1") 到 #secref("9.6") 里，你面对的不是确定答案，而是如何从不完整的信息里做出更稳的判断。
