...
#table(
  ...
  [总结], [5], [在 #secref("5.1") 与 #secref("5.2") 里确认旧工具已经能混合调用，再决定进入哪一条专题深化路线。],
...
#block(inset: (left: 1em), stroke: (left: 2pt + rgb("#F57F17")))[
  #text(size: 9pt)[
    这张图故意把第五章画成一次“总结”，而不是第五个继续往前排的普通节点：它的任务是把共同地基收束、检查、分流。第六到第九章也故意并排出现，因为它们是*并列的专题深化路线*，不是一条旧式单行道上的连续车站。
  ]
]

=== 每条专题深化路线都在重复同一种发明动作

- *代数深化*：在 #secref("6.1") 到 #secref("6.10") 里，你不断把“关系写出来再操作”的能力做得更稳，从模板走到求解，从求一个数走到限制一整个范围。
- *几何深化*：在 #secref("7.1") 到 #secref("7.13") 里，你把“看起来对”推进成“必须如此”，又把测量问题接到相似、勾股、三角比与场景测量上；这里的三角比在 #secref("7.9")，而仰角、俯角、坡度等应用安放在 #secref("7.10")，一起组成几何测量链的一部分。
- *函数模型*：在 #secref("8.1") 到 #secref("8.5") 里，你不断比较“怎样变化”，把比例、坐标、公式和图象收进同一个模型视角。
- *数据统计*：在 #secref("9.1") 到 #secref("9.8") 里，你面对的不是确定答案，而是如何从不完整的信息里做出更稳的判断。
