...
#discovery[
  *先用旧工具试试。* 全等判定已有 SSS、SAS 等工具，相似感觉只是"放大了的全等"，那就照搬 SSS 的思路：核对三组对应边之比是否等于同一个 $k$，再验三组对应角是否相等——这样最保险。
  ...
  *这里卡住了。* 核对 6 个量（3 边比 $+$ 3 角）和全等一样繁琐；实际题目往往只给出几个角或几条边，剩下的既无法量到也无法算出。我们需要一条*只凭几个角度就能宣布相似*的省力判定法。
...
#pitfall[
  *高频错误*
  ...
  脑子里把“相等 → 全等”那条短路径拉了过来，跳过了“比例 $= 1$”这一关。
