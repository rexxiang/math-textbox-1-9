...
#mastery[
  ...
  *挑战*
  ...
  + *[反直觉]* 判断："如果两条直线 $ell_1$、$ell_2$ 与第三条线 $t$ 形成的某一对角相等，就一定 $ell_1 parallel ell_2$。" 给反例（例如对顶角永远相等，但与平行性无关）；并指出"哪些角相等才能推平行"。
  + *[跨知识点]* 已知 $A B parallel C D$，$angle E A B = 30°$，$angle E C D = 50°$，点 $E$ 在 $A B$ 与 $C D$ 之间。求 $angle A E C$。提示：过 $E$ 作 $A B$（即 $C D$）的平行线，再用本节平行线的内错角 / 同位角，并呼应 #secref("tool:pf08-triangles-angle-sum") 的"角度可以拼接"。
  + *[构造题 / 反例]* 给出三条直线 $ell_1, ell_2, ell_3$ 的位置安排，使得 $ell_1 parallel ell_2$ 且 $ell_1 parallel ell_3$，但 $ell_2$ 与 $ell_3$ 是同一条直线（重合）。再给出一个安排使得 $ell_2$ 与 $ell_3$ 不重合且都平行于 $ell_1$。说明"平行的传递性"在这两种情况下分别表现为什么。
  ...
  *挑战*
  ...
  + 反例：在交点处，对顶角永远相等（如两条相交直线交出的对顶角 $angle 1 = angle 3$、$angle 2 = angle 4$），但此时两条直线根本不是平行而是相交。"角相等推平行"的标准是*同位角相等 / 内错角相等*（或同旁内角互补），且这一对角必须分别在被截两线上、且位置形态符合三种规定。任意两个相等的角不能推平行（角的位置 / 形态决定能否做平行判据）。
  + 过 $E$ 作 $ell parallel A B$（即 $ell parallel C D$）。$ell$ 把 $angle A E C$ 分成上下两半 $angle A E P$ 与 $angle P E C$。由内错角：$angle A E P = angle E A B = 30°$（$ell parallel A B$，$E A$ 为截线）；$angle P E C = angle E C D = 50°$（$ell parallel C D$，$E C$ 为截线）。所以 $angle A E C = 30° + 50° = 80°$（呼应 #secref("tool:pf08-triangles-angle-sum") 的"角度拼接"思想）。
  + 重合情形：$ell_2$ 与 $ell_3$ 写法不同但其实是同一条直线（如 $y = 2$ 与 $2 y = 4$），都平行于 $ell_1: y = 0$。不重合情形：$ell_1: y = 0$，$ell_2: y = 1$，$ell_3: y = 2$，三条互相平行，$ell_2 != ell_3$。第一种情形里"传递性"退化为同一条直线（任何直线"和自身平行"按约定通常成立或视约定）；第二种情形是真正的传递：$ell_2 parallel ell_1, ell_3 parallel ell_1 ⇒ ell_2 parallel ell_3$（且不重合，平行于同一直线的两条直线要么重合、要么平行）。
  ]
]
