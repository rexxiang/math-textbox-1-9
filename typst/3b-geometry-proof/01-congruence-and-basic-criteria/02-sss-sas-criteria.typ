...
#discovery[
  *先用旧工具试试。* 全等的定义说"三边三角一一对应相等"——6 个条件。来试一道题：$triangle A B C$ 中 $A B = 5$，$B C = 4$，$A C = 6$；$triangle D E F$ 中 $D E = 5$，$E F = 4$，$D F = 6$。先核对三条边：$A B = D E = 5$ ✓，$B C = E F = 4$ ✓，$A C = D F = 6$ ✓——三边都满足。

  试着推下去——

  按定义还需要核对三个角：$angle A = angle D$？$angle B = angle E$？$angle C = angle F$？题目根本没给任何角度信息。要量角得先作图，要计算角度得用余弦定理——可余弦定理还没学，陷入循环等待。

  *这里卡住了。* 定义要求 6 项全部核对，但实际题目常常只给部分信息，剩下的既算不出又量不到；如果三边已经能锁死三角形形状，再去"验"三个角就是白费功夫。我们需要一张"最短清单"——只核*几项*就能宣布全等。
