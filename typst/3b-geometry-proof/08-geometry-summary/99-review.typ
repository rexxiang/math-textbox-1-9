...
#blueprint[
  *过关信号总清单*
  ...
  五条都打勾，接下来可以选择进入函数与变化、数据与不确定性，或返回代数分支做更深的串联——前面学的“几何直觉 + 坐标化语言”会在三条主线里反复出现。
]
...
#side-hack[
  *☞ 跨部分回看*
  ...
  // 跨章回顾
  + 两条平行线被截后，内错角为 $(2 x + 10) degree$ 和 $(3 x - 20) degree$。列方程（#secref("cb06-equation-as-balance")），解出 $x$，再说明这两个角各是多少度。
  // 跨章回顾
  + 一个三角形的三边长分别是 $a$、$a + 2$、$a + 4$。如果周长为 $36$，先用代数解出 $a$（#secref("al04-linear-one-variable")），再判断这个三角形是否为直角三角形（#secref("ge04-pythagorean")）。
  // 跨章回顾
  + 坐标系中有 $A(0, 0)$、$B(6, 0)$、$C(6, 8)$。计算 $triangle A B C$ 的面积，再用百分数说明 $triangle A B C$ 的面积是外接矩形面积的多少百分比。
]
...
#side-hack[
  *☞ 高中桥 / 继续往前*
  ...
  - *解析几何*：把直线、圆、圆锥曲线都写成方程，几何问题彻底变成代数问题。前面学的距离 / 中点 / 变换公式会作为最基础的工具反复登场。
  - *向量*：把“从 $P$ 到 $Q$ 的位移”命名为向量；平移、旋转、反射用矩阵统一表示。前面学的刚体变换是其几何雏形。
  - *三角*：前面只开了 $30 degree$ / $45 degree$ / $60 degree$ 三个口子，高中用单位圆把三角比推广到任意角；随之而来的一般三角形主定理把“直角”这个限制拿掉。
  - *空间几何*：三视图是最朴素的投影；再往前走是透视变换、空间向量、线性代数与计算机图形学。

  几何部分到这里正式结束。下次当你打开任何一本几何教科书，会发现它的目录和前面的知识地图高度吻合——从“图形直观”到“坐标化与变换”这条线索，是几何学两千多年来的共同骨架。
]
