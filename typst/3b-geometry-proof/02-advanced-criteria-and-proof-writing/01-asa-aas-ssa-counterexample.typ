...
#crisis[
  证明题很少把条件送得刚刚好。你可能只知道两个角和一条边：两条射线的方向都定了，可那条边到底放在中间还是放在旁边，结论会不会变？

  另一个更容易让人手滑的场景是：两条边都知道了，还给了一个角，但这个角没有夹在两条边中间。它看起来只比 SAS 差一点点；差的这一点，会不会让同一组条件画出不同的三角形？

  这一节先把这两个困境摆清楚：*两角一边到底够不够？两边一非夹角为什么会骗人？*
]
...
#self-check[
  如果一个角没有夹住两条已知边，另一端点还能不能“绕”出第二个位置？先用这句话解释，再看下面证明。
]



#side-hack[
  *完整示例*——把 ASA / AAS 写成证明，再看 SSA 为什么不够

  *ASA / AAS 证明怎么写*

  + *看题*：已知 $angle A = angle D$，$angle B = angle E$，$A B = D E$。求证 $triangle A B C tilde.eq triangle D E F$。
  + *选工具*：这是"两角及其夹边"相等，用 `ASA`。若给的是不夹在两角之间的边，则先用第三角相等把它转成 `AAS` 或 `ASA`。
  + *动手*：在 $triangle A B C$ 与 $triangle D E F$ 中，
    - $angle A = angle D$（已知）；
    - $A B = D E$（已知，且是两角夹边）；
    - $angle B = angle E$（已知）。

    所以 $triangle A B C tilde.eq triangle D E F$（`ASA`）。若把 $A B = D E$ 换成 $A C = D F$，则先由内角和得 $angle C = angle F$，再用 $angle A$、$A C$、$angle C$ 判定。
  + *回看*：证明里必须写清"在哪两个三角形中"、三条对应条件和所用判据，不能只写"显然全等"。

  *短反例：SSA 为什么不够*

  + *看题*：固定 $A B = 7$、$B C = 5$、$angle A = 30 degree$，角 $A$ 不是两边的夹角。
  + *选工具*：这落在 `SSA` 警戒区，不能直接套 `SAS`。
  + *动手*：从 $A$ 作一条与 $A B$ 成 $30 degree$ 的射线，再以 $B$ 为圆心、$5$ 为半径画圆。圆可能与这条射线相交于两个不同点 $C_1, C_2$。
  + *回看*：两个三角形都满足同样的两边和一个非夹角，但第三边不同，所以不一定全等；`SSA` 不能作为判定法。
]
...
#pitfall[
  *高频错误：把 SSA 当成全等判定法*
  ...
  #block(inset: (left: 12pt))[
    已知 $triangle A B C$ 与 $triangle D E F$ 中，$A B = D E = 5$，$B C = E F = 7$，$angle A = angle D = 40 degree$。

    第 1 行：有两条边相等，还有一个角相等。

    第 2 行：所以“两边一角相等”。

    第 3 行：$therefore triangle A B C tilde.eq triangle D E F$（SSA）。$quad$ ✗
  ]

  ✗ 错处：第 3 行。这里的 $angle A$ 不是 $A B$ 与 $B C$ 的夹角；它是“两边一非夹角”。固定 $A B = 5$，从 $A$ 作 $40 degree$ 射线，再以 $B$ 为圆心、半径 $7$ 画弧，弧可能与射线交于两个不同点——条件相同，形状不同。

  ✓ 改法：相等角若是两条已知边的*夹角*，才用 `SAS`；若能再补一组对应角，就改用 `ASA` 或 `AAS`；若只停在 SSA，结论应写“条件不足，不能判定全等”。

  *自查句*：看到“两边一角”，立刻问：“这个角夹在两条已知边中间吗？”不在中间，就进入 SSA 警戒区。

  ✗ “两角相等”即断全等 → ✓ 两角相等只能说形状相似，还需配一条对应边。

  ✗ AAA 判全等 → ✓ AAA 只能说明形状相同，大小未定。
]
...
#mastery[
  ...
  *挑战*
  ...
  + *[跨知识点]*：`HL` 在本节翻成 `AAS`。如果改用 #secref("tool:ge04-pythagorean") 的勾股定理：已知斜边 $c$ 与一条直角边 $a$，则另一条直角边 $b = sqrt(c^2 - a^2)$ *唯一确定*。请用这一点直接说明：在直角三角形中，"斜边 $+$ 一条直角边"等价于 `SSS`。再给出一组 $c = 13$、$a = 5$ 的具体数据验证。
