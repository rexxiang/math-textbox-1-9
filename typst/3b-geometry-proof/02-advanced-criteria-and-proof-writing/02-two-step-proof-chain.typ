...
上一节我们看到 ASA / AAS 给了我们更多判定全等的工具——但很多题目里，"够条件"不会一下子全摆好。比如条件给的是"$A B = C D$ 和 $angle B = angle D$"，要证 $triangle A B X tilde.eq triangle C D X$，可能还差一组对应边或角。这时候需要先用其他已知条件*推出*那一组，再用 SSS / SAS / ASA 收尾——这就是证明链。

本节就教你怎么把"先推出 → 再判定"这两步衔接起来写。
  ...
  + 点 $P$ 在 $angle A O B$ 的内部，过 $P$ 向 $O A$、$O B$ 作垂线，垂足为 $M$、$N$。若 $P M = P N$，求证 $O P$ 平分 $angle A O B$。（提示：先证 $triangle O P M tilde.eq triangle O P N$。）
    ...
    #side-hack[
      *思路骨架*（卡住了再看下面的步骤）：
      ...
      + 第 1 步：先把要证的目标拆开——“$O P$ 平分 $angle A O B$” 等价于 “$angle M O P = angle N O P$”，所以最终需要的是*一对对应角相等*（先描述目标，暂不求解）。
      + 第 2 步：要拿到这对角相等，最稳的方法是凑*一对全等三角形*，让 $angle M O P$、$angle N O P$ 成为对应角。
      + 第 3 步：数清两个三角形 $triangle O P M$ 与 $triangle O P N$ 的对应条件——一对直角（垂线定义）、一对相等的腰（$P M = P N$ 已知）、一条公共斜边（$O P$），共 $3$ 个条件。
      + 第 4 步：选判据——直角 $+$ 斜边 $+$ 一直角边对应相等可用 `HL`（或先由勾股推第三边相等再用 `SSS`）；写两栏证明，最后读出对应角即可。
