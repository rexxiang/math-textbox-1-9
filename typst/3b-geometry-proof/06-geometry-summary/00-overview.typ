...
= 几何支总结 <ch:geometry-summary>

#blueprint[
  *难度*：★★★★☆ \
  *核心问题*：从“图形直观”一路到“三角比、变换与视图”，这条几何分支的发明链到底是怎么一步一步长出来的？每一章到底发明了*哪一个新工具*来解决上一章无法解决的问题？若中途某处卡住，该回哪一章？ \
  ...
  *本章在全书的位置*：几何分支*收束*——不发明新工具，而是把全支的工具串起来看清逻辑依赖。 \
  *忘了可以回到*：本章 §3 会给出“分支学完后最常见的误读”与对应回补节。
]

#side-hack[
  *本章路线*

  + §1 画出几何支的发明树：从“图形直观（点 / 线 / 角 / 多边形 / 圆）”如何一步步被加结构，最终长出“全等 → 相似 → 四边形与圆 → 勾股 & 三角比 → 变换 & 视图”。每个工具都带着 `#secref` 回到本章。
  + §2 出 $3$ 道*跨章综合题*——每题都显式注明它调用了哪些工具链。
  + §3 梳理分支学完之后最常见的*误读*与对应的*回补路径*——只重读某一节，不重刷整章。

  *这章通向哪里*

  - 代数分支：几何题里“设未知边 / 角”求值，就是代数分支的方程 / 不等式再利用。
  - 函数分支：坐标系里“点的轨迹”就是一个函数关系；直线、圆都是最早的例子。
  - 数据分支：几何图形（柱状图、饼图、散点图）的面积比较与坐标位置，靠本支工具解读。
