...
#discovery[
  *先用旧工具试试。* 圆心角 $angle A O B = 80°$，想用"三角形内角和 $= 180°$"来推圆周角 $angle A C B$。感觉圆心角和圆周角"面对同一段弧"，也许互补？$180° - 80° = 100°$？

  试着推下去——

  取两组具体数值验证：令 $angle A O B = 60°$，在圆上取两个位置 $C_1$、$C_2$，用量角器量出 $angle A C_1 B approx 30°$、$angle A C_2 B approx 30°$——两者相等，但都是 $30°$，根本不是"互补"猜测的 $180° - 60° = 120°$。猜测的几何依据完全失效。

  *这里卡住了。* 三角形内角和只能说"三个角加起来 $180°$"，根本无法直接比较圆心角和圆周角的大小关系；凭"互补"的直觉猜测没有任何几何依据。我们需要一条专门把*同弧的两类角联系起来*的定理。
