...
#mastery[
  ...
  *挑战 ☞ 选做*
  ...
  + *反直觉*：仰角越大，看到的物体一定越高吗？请构造一个反例，说明同一物体可能因*距离不同*而对应不同的仰角。
  + *跨知识*：一段斜坡坡角 $= 30 degree$、坡长 $= 12$ 米。求它的*水平投影长*与*铅直高度*；再写出坡度（铅直 $:$ 水平）的最简比。（跨三角比 + 坡度定义）
  + *构造题*：构造一个具体情境，使俯角 $= 30 degree$、水平距离 $= 50$ 米能算出观察者所在的最低高度。要求写出情境（如登塔观察地面物体），并求出最低高度。
  ...
  *挑战 ☞ 选做*
  ...
  + 反例：同一面 $10$ 米高的旗杆，从距旗杆底 $10$ 米处测得仰角 $tan theta_1 = 10 / 10 = 1 -> theta_1 = 45 degree$；从 $30$ 米外测得 $tan theta_2 = 10 / 30 = 1 / 3 -> theta_2 approx 18.43 degree$。同一物体（同高），仰角随*观察者位置*改变——*仰角大小不直接代表物体高度*，要结合距离才能比较。
  + 坡长 $12$、坡角 $30 degree$：水平投影 $= 12 cos 30 degree = 12 dot sqrt(3) / 2 = 6 sqrt(3) approx 10.39$ 米；铅直高度 $= 12 sin 30 degree = 12 dot 1 / 2 = 6$ 米。坡度 $=$ 铅直 $:$ 水平 $= 6 : 6 sqrt(3) = 1 : sqrt(3) approx 1 : 1.732$。
  + 情境：观察者站在塔顶水平望出，看到地面上一个物体的俯角 $= 30 degree$、水平距离 $= 50$ 米；求观察者最低的离地高度（即塔高）。设塔高 $h$，则 $tan 30 degree = h / 50 -> h = 50 tan 30 degree = 50 sqrt(3) / 3 approx 28.87$ 米。要点：俯角 + 水平距离 → 高度（仰角 / 俯角 + tan 模型）。
  ]
]
