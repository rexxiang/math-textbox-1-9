...
#discovery[
  对每个 $45°$–$45°$–$90°$ 三角形，用勾股定理重算：直角边 $k$，斜边 $= sqrt(k^2 + k^2) = k sqrt(2)$；直角边 $5$ → 斜边 $= 5 sqrt(2)$；直角边 $7$ → 斜边 $= 7 sqrt(2)$；直角边 $sqrt(3)$ → 斜边 $= sqrt(6)$——每次都得重算一遍开方。
  ...
  用勾股定理从头算没问题，但对角度固定的"特殊"三角形，每次都重复同样的开方计算，且没有任何固定比例可以直接读答案——同一种角度的三角形，做十道题就要重算十遍同样的根号。
