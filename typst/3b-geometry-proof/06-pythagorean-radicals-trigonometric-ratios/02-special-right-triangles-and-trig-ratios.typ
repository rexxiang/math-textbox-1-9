...
#crisis[
  勾股定理给出“斜边 $=$ 两直角边平方和再开方”。可只要两直角边长度任换一组，就得再开一次方。有没有两族*形状完全固定*的直角三角形——它们的三边比例一次定下来以后就不用再算？这样一来，只要知道*一条边 + 一个角*，其余两条边就能直接按比例写出来。
...
#discovery[
  *先用旧工具试试。* 对每个 $45°$–$45°$–$90°$ 三角形，用勾股定理重算：直角边 $k$，斜边 $= sqrt(k^2 + k^2) = k sqrt(2)$；直角边 $5$ → 斜边 $= 5 sqrt(2)$；直角边 $7$ → 斜边 $= 7 sqrt(2)$；直角边 $sqrt(3)$ → 斜边 $= sqrt(6)$——每次都得重算一遍开方。

  试着推下去——

  再遇到"$angle B = 60°$，直角边 $B C = 8$，求斜边 $A B$"：需要先判断 $B C$ 是 $60°$ 的对边还是邻边，建立方程 $B C = A B sin 60°$ 或 $B C = A B cos 60°$——但 $sin 60°$ 和 $cos 60°$ 是什么？每次都要从勾股定理算一遍，步骤三四步，而且每次还要重新搞清楚"这条边是对边还是邻边"。

  *这里卡住了。* 用勾股定理从头算没问题，但对角度固定的"特殊"三角形，每次都重复同样的开方计算，且没有任何固定比例可以直接读答案。我们需要把两族特殊三角形的*三边固定比*提炼出来，让答案可以直接由已知边乘以比例系数得到。
...
#mastery[
  ...
  *挑战*
  ...
  + *[反直觉]*：判断："$45 degree + 45 degree = 90 degree$，所以 $sin 45 degree + sin 45 degree = sin 90 degree$。" 计算两边验证（$2 dot sqrt(2)/2 = sqrt(2) approx 1.414$ vs $sin 90 degree = 1$），并解释*三角函数对加法不分配*——这正是 #secref("tool:cb03-rational-laws") 中"$f(a + b) != f(a) + f(b)$" 的另一例。

  + *[构造题]*：构造一个直角三角形，使它的一个锐角的正切值为 $sqrt(3) / 3$。给出三边并验证。再说明这个角是几度。

  + *[找 Bug]*：小李在 $30$–$60$–$90$ 三角形中，看到 $angle A = 30 degree$ 的对边长 $5$，写出"斜边 $= 5 / 2$"。他错在哪？正确斜边是？哪条边比是被弄反的？

  #answer-cut[

  *基础*
  + 斜边 $= 7 sqrt(2)$（45-45-90 三角形）。
  + 长直角边 $= 3 sqrt(3)$；斜边 $= 6$（30-60-90 三角形）。
  + $sqrt(3) slash 2 - sqrt(3) slash 2 = 0$（特殊角三角比）。
  ...
  *挑战*
  ...
  + 错。$2 sin 45 degree = sqrt(2) approx 1.414 != 1 = sin 90 degree$。三角函数*不对加法分配*：一般 $sin(a + b) != sin a + sin b$（实际有"和角公式" $sin(a + b) = sin a cos b + cos a sin b$）。这与 #secref("tool:cb03-rational-laws") 的"函数不分配"是同样的警告。
  + 锐角 $alpha$ 的对边 / 邻边 $= sqrt(3)/3 = 1/sqrt(3)$，对应 $30$–$60$–$90$ 中 $30 degree$ 那个角（$tan 30 degree = sqrt(3)/3$）。一例：直角三角形三边 $1 : sqrt(3) : 2$（短边 $1$、长直角边 $sqrt(3)$、斜边 $2$）。验证：$tan 30 degree = "对" / "邻" = 1 / sqrt(3) = sqrt(3) / 3$ ✓。$alpha = 30 degree$。
  + 错。$30$–$60$–$90$ 边比是 *短直角边 $:$ 长直角边 $:$ 斜边 $= 1 : sqrt(3) : 2$*，$30 degree$ 的对边是*短直角边*（$= 5$），所以斜边 $= 2 times 5 = 10$，不是 $5 / 2$。他把"斜边 $=$ 短边 $div 2$"和"短边 $=$ 斜边 $div 2$"弄反了——后者才对。
  ]
]
