...
#tryit[
  + 计算：$2/5 + 3/4$（先把你的第一直觉答案写下来，再仔细算一次）。
  + 计算：$4/5 times 5/8$。
]
...
#pitfall[
  ...
  #block(width: 100%, fill: rgb("#FFFDE7"), radius: 2pt, inset: 8pt)[
    *卡在哪里*：第 1、2 行都套了"整数按位相加"的旧规则，但分数的*分母不是位，是单位*——不同分母代表不同大小的份。$1/4$ 和 $2/3$ 的"份"压根不一样大，不能直接合并。

    粗估一下就露馅：$2/3 approx 0.67$，$1/4 = 0.25$，合应 $> 0.67$；可 Mei 的答案 $3/7 approx 0.43$，比被加数还小，明显荒谬。

    *正确做法*：先通分到同一单位（公分母 $12$）：
  ...
  ✗ 乘法也去通分 → ✓ 乘法是"求部分中的部分"，不需要统一单位。

  ✗ 除法只会机械背"倒过来乘"，却不知道意思 → ✓ 把除法理解为"里面有几个这样的分数单位"。
]
  ...
  + 计算 $frac(1, 1 times 2) + frac(1, 2 times 3) + frac(1, 3 times 4) + frac(1, 4 times 5)$。（提示：试试 $frac(1, n(n+1)) = frac(1, n) - frac(1, n+1)$，再观察裂项相消。）
    ...
    #side-hack[
      *思路骨架*（卡住了再看下面的步骤）：
      ...
      + 第 1 步：观察通项 $frac(1, n(n+1))$——能不能拆成两个更简单分数的差？试着假设 $frac(1, n(n+1)) = frac(A, n) + frac(B, n+1)$，通分回去对照系数。
      + 第 2 步：验证你的拆分：$frac(1, n) - frac(1, n+1) = frac((n+1) - n, n(n+1)) = frac(1, n(n+1))$ ✓。
      + 第 3 步：把 $4$ 项都改写成"两分数之差"，写出来后看哪些项相互抵消。
      + 第 4 步：剩下首尾两项相减，化简得到答案。
