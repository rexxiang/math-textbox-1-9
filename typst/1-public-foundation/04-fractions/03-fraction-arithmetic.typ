...
#discovery[
  *先用旧工具试试。* 整数加法的规律是"同位数字对应相加"——个位加个位，十位加十位。把这套规则套到分数：$1/4 + 2/3$，分子相加 $1 + 2 = 3$，分母相加 $4 + 3 = 7$，得到 $3/7$。
  ...
  *这里卡住了。* 分母不是"位"，是"份"的大小；不同分母代表不同大小的份，不能像整数按位直接叠加——我们需要先把两个分数换成同一种"份"，才能把份数相加。
