...
#discovery[
  整数加法的规律是"同位数字对应相加"——个位加个位，十位加十位。把这套规则套到分数：$1/4 + 2/3$，分子相加 $1 + 2 = 3$，分母相加 $4 + 3 = 7$，得到 $3/7$。

  试着推下去——

  检验 $3/7$ 是否合理：$2/3 approx 0.67$，$1/4 = 0.25$，两者相加应超过 $0.67$；但 $3/7 approx 0.43$，比被加数 $2/3$ 还小——加了东西反而变少，明显荒谬。再用整数类比验证：$1/1 + 1/1$，按同样规则得 $2/2 = 1$——可两个"完整的一"加起来应该是 2，不是 1。

  分母不是"位"，是"份"的大小；不同分母代表不同大小的份，不能像整数按位直接叠加，硬加只会得到一个谁也说不清意思的结果。
...
#pitfall[
  *高频错误*
  ...
  ✗ 异分母加法时把分子、分母各自相加
  → ✓ 分母不同先通分，否则单位没统一。
  把整数加法里“按位对应相加”的动作直接平移过来了——但分母不是位，是单位。
...
#mastery[
  ...
  *挑战*
  ...
  + *推广反推题：从特例到一般，再反向求解*
    - *特例*：先算 $3/4 div 1/2$。结果你已经知道是 $3/2$。
    - *推广*：把"$1/2$"换成任意分数 $a/b$（$a > 0, b > 0$）。$3/4 div (a/b)$ 等于多少？请用代数式写出，并解释为什么"除以分数 $=$ 乘以倒数"对所有 $a/b$ 都成立。
    - *反推*：现在反过来——*已知* $3/4 div (a/b) = 1$，能反求出 $a/b$ 吗？再问：若 $3/4 div (a/b) = 6$，又是多少？请说出你用了什么"反向操作"。

  + 能否找到三个"单位分数"（分子为 $1$ 的分数，允许重复），使它们之和恰好等于 $1$？至少给出两组不同的组合。
  + 计算 $frac(1, 1 times 2) + frac(1, 2 times 3) + frac(1, 3 times 4) + frac(1, 4 times 5)$。（提示：试试 $frac(1, n(n+1)) = frac(1, n) - frac(1, n+1)$，再观察裂项相消。）
  + *[构造题]*：构造一个算式 $frac(a, b) div frac(c, d) = 2$，要求 $a, b, c, d$ 是四个不同的个位正整数。给出两种不同的构造。
...
#answer-cut[
  ...
  + *推广反推*：
    - 特例：$3/4 div 1/2 = 3/4 times 2/1 = 3/2$。
    - 推广：$3/4 div (a/b) = 3/4 times (b/a) = (3 b) / (4 a)$。无论 $a, b$ 是什么正分数，"除以一个分数 $=$ 乘以它的倒数"都成立——因为"$3/4$ 里面有几个 $a/b$"等价于"$3/4$ 是 $a/b$ 的几倍"，倒数 $b/a$ 把"问几倍"翻译成了乘法。
    - 反推：若 $3/4 div (a/b) = 1$，则 $(3 b) / (4 a) = 1$，得 $3 b = 4 a$ $=>$ $a/b = 3/4$（除自己得 $1$）。若 $3/4 div (a/b) = 6$，则 $(3 b) / (4 a) = 6$，得 $b / a = 8$，即 $a/b = 1/8$。验证：$3/4 div 1/8 = 3/4 times 8 = 6$ ✓。反向操作 $=$ "把除变乘 $=>$ 解一次方程 $=>$ 再翻回分数"，本质是把除法当作"乘法的逆"。
  + 三个单位分数之和等于 $1$ 的组合：$frac(1, 2) + frac(1, 3) + frac(1, 6) = 1$（分母不同的唯一解）；$frac(1, 2) + frac(1, 4) + frac(1, 4) = 1$；$frac(1, 3) + frac(1, 3) + frac(1, 3) = 1$。若要求分母两两不同，只有 $frac(1,2)+frac(1,3)+frac(1,6)$ 这一组（可用穷举法验证：固定最小分母 $a=2$，则 $frac(1,b)+frac(1,c)=frac(1,2)$，$b=3 => c=6$ 是唯一解；$b=4 => c=4$ 重复）（单位分数凑整 + 分类讨论）。
  + 裂项：$frac(1,n(n+1))=frac(1,n)-frac(1,n+1)$。各项相消：$(1-frac(1,2))+(frac(1,2)-frac(1,3))+(frac(1,3)-frac(1,4))+(frac(1,4)-frac(1,5))=1-frac(1,5)=frac(4,5)$（裂项相消法）。
  + *[构造题]*：需要 $frac(a, b) div frac(c, d) = 2$，即 $frac(a d, b c) = 2$，即 $a d = 2 b c$，且 $a, b, c, d$ 四个不同个位正整数。构造例一：取 $a = 8, b = 6, c = 2, d = 3$：$frac(8, 6) div frac(2, 3) = frac(8, 6) times frac(3, 2) = frac(24, 12) = 2$ ✓，四个数 $8, 6, 2, 3$ 各不同 ✓。构造例二：取 $a = 6, b = 9, c = 1, d = 3$：$frac(6, 9) div frac(1, 3) = frac(6, 9) times 3 = frac(18, 9) = 2$ ✓，四个数 $6, 9, 1, 3$ 各不同 ✓（除以分数 $=$ 乘以倒数 + 调参构造）。
]
]
