...
#discovery[
  *先用旧工具试试。* 比较两个分数，自然想到直接看数字：$3/4$ 和 $2/3$，分子 $3 > 2$，所以 $3/4$ 更大？但分母 $4 > 3$，每一份反而更小，所以 $3/4$ 更小？两条"直觉规则"给出矛盾的答案。
  ...
  *这里卡住了。* 分母不同时，每份的大小也不同，直接比较"几份"毫无意义——我们需要一种先把两个分数换成同一种"份"再比较的方法。
