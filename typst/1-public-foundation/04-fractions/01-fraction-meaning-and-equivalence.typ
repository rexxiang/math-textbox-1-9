...
#crisis[
  ...
  *我们需要一种数*：(1) 能稳定地描述"整体的一部分"；(2) 同一个量在*不同切法*下能被认出是*同一个量*；(3) 可以和别的同类数字直接比较。
]

#discovery[
  *尝试 A：用语言形容*

  "比一半多一点"、"差不多三分之二"——两块田都"比一半多一点"，可一块明显比另一块大。语言形容*太粗*，不可比较。

  *尝试 B：标准化"切法 $+$ 占了几份"*

  统一约定：把"$1$ 个标准整体"*平均切成 $b$ 份*（强调*平均*——每份必须一样大），然后说"占其中的 $a$ 份"。把这个数写作
