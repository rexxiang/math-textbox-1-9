...
#discovery[
  *先用旧工具试试。* 我们只有整数——测量一根木板，结果比 1 肘长、又不到 2 肘，整数只能含糊地写"1"或"2"。改用分数？超出部分写成 $1/3$ 肘或 $2/5$ 肘，每次要重新选分母，而且 $1/3$ 和 $2/5$ 精度完全不同。
  ...
  *这里卡住了。* 分数能表示不足 1 的量，但分母每次都要另选，跟整数"相邻两位差十倍"的位值结构脱节——我们需要一种能让位值系统自然向右延伸、统一表示十分之一、百分之一……的写法。
