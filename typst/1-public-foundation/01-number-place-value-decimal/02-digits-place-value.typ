...
#discovery[
  *先用旧工具试试。* 我们手里只有 0 到 9 这十个数字符号，自然会想："数字就是数字，2 在哪里都代表 2。"用这条想法比较 2、20、200——三个数里都有数字 2，按"数字决定大小"来判断，三者应该差不多大。
  ...
  *这里卡住了。* 只认数字的形状，根本无法区分同一个数字站在不同格子里代表的大小——我们需要一种让"数字所处位置"决定其大小的写法。
