...
#tryit[
  + 两个盒子底面都是 $4 times 3$，A 高 $5$、B 高 $2$。它们的装沙量一样吗？先凭直觉给个判断，再算一次。
]


#discovery[
  我们已经有面积——底面能铺多少块单位正方形。一个长 4、宽 3 的盒子底面积是 $4 times 3 = 12$ 平方单位。用底面积来回答"这个盒子能装多少沙子"？两个盒子都是 $4 times 3$ 的底面，按底面积，两者"装沙能力"一样。
  ...
  面积只量了底面这一层，完全忽略了"有几层"这件事；同样底面的盒子越叠越高时面积一动不动，可装的水却差得很远。
