...
#discovery[
  *先用旧工具试试。* 我们会列因数——找 24 和 36 的公因数，把两者的因数全列出来再找重叠：$24$ 的因数：$1, 2, 3, 4, 6, 8, 12, 24$；$36$ 的因数：$1, 2, 3, 4, 6, 9, 12, 18, 36$。重叠部分最大是 $12$。列表法算出来了。
  ...
  *这里卡住了。* 逐一列举因数和倍数在稍大的数面前就算不动，而且看不出两数"共享哪些乘法结构"——我们需要一种直接抓住两数共同质因数成分的系统方法。
