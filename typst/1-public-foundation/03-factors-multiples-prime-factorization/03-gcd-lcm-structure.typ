...
#side-hack[
  *两条路对比：列举因数（倍数）法 vs 质因数分解法*

  #table(
    columns: (auto, 1fr, 1fr),
    inset: 6pt,
    stroke: 0.4pt,
    [*情境*], [*方法 A：列举法*], [*方法 B：质因数分解法*],
    [何时更快], [两数均 $<= 30$（如 $gcd(12, 18)$）：$12$ 的因数 $1, 2, 3, 4, 6, 12$；$18$ 的因数 $1, 2, 3, 6, 9, 18$；交集最大值 $= 6$，不超过 $10$ 秒], [至少一个数 $> 30$（如 $gcd(60, 84)$）：$60 = 2^2 dot 3 dot 5$，$84 = 2^2 dot 3 dot 7$，$gcd = 12$，$"lcm" = 2^2 dot 3 dot 5 dot 7 = 420$，一次分解同时得出两个答案],
    [典型陷阱], [列举因数时漏掉配对：$24$ 的因数应按 $1 times 24, 2 times 12, 3 times 8, 4 times 6$ 逐对列出，$8$ 最容易漏掉], [分解时次幂写错：$36 = 2^2 times 3^2$ 不是 $2^2 times 3^3$；$gcd$ 取"较小次幂"，$"lcm"$ 取"较大次幂"——两个方向相反，容易混],
    [建议先用], [两数均 $<= 30$ 时列举法直接；只求 $gcd$ 或只求 $"lcm"$ 时列举也够快], [两数较大或需要同时得出 $gcd$ 和 $"lcm"$ 时，质因数分解一次作完，再分别取较小 / 较大次幂，不用重做一遍],
  )

  *经验法则*：两数 $<= 30$ 且只求一个答案，列举法最快；至少一个数 $> 30$ 或需要同时得出 $gcd$ 和 $"lcm"$，质因数分解结构清晰，一步两得。
]
...
#history-note[
  *王小云（$1966$–）*——中国密码学家，山东大学教授，中国科学院院士。她在 $2004$–$2005$ 年带领团队公开了对 MD$5$ 与 SHA-$1$ 两大主流哈希函数的有效碰撞攻击，迫使国际密码学界全面更新标准。许多公钥密码体系的安全性，归根结底建立在"两个大质数相乘容易、把乘积分解回去极难"这个事实上——也就是你这节学到的*因数*与*最大公因数*的极端版本。一个看似初等的概念，被推到极限就成了信息时代的护城河。
]
