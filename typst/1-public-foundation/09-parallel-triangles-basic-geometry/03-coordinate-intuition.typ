...
#crisis[
  ...
  如果位置能被稳定地写成一对数，你一报出来，对方就能在图上精确找到你——“一对数 = 一个位置”这种对应，能被造出来吗？
]
#tryit[
  先动手试试，下面再读。

  + 在方格纸上随意标出 3 个点（不需要轴），发明一套自己的编码规则，给每个点写出一对数，使得只凭这两个数，别人也能在空白方格纸上精确画出你的点。你的规则是什么？有没有歧义？
]

#discovery[
  *先用旧工具试试。* 数轴已经能用一个数精确定位一条线上的点。把这套方法搬到操场：以大门为起点，"向东 5 步"就是一个数 $5$，可以唯一确定位置——可以吗？

  试着推下去——

  "向东 5 步"意味着：从大门出发，沿东方向走 5 步——结果是操场正东方的一个点。但操场上偏北 3 步、偏南 2 步的位置，距大门也都是"向东 5 步的水平分量"，说"5"根本无法唯一确定一个点。改成"距大门 5 步"：以大门为圆心，半径 5 的整个圆上每个点都满足，定位更不唯一。再加方向"偏东 30 度"可以唯一定位——但两个点 $(5,30°)$ 和 $(8,45°)$ 的中点是什么？$((5+8)/2, (30°+45°)/2) = (6.5, 37.5°)$——这不是几何中点，需要三角函数才能算出来。

  *这里卡住了。* 一个数只够在一条线上定位；用"距离 + 角度"可以唯一标注点，但加减中点等基本计算立刻需要三角函数——我们需要一种用两个独立的数描述平面上每个点、并让中点等运算只用加减就能完成的新方案。
...
#mastery[
  ...
  *挑战 ☞ 选做*
  ...
  + *反直觉*：$(2, 3)$ 和 $(3, 2)$ 离原点一样远吗？请用勾股直觉（横平 + 竖直走法）说明。
  + *跨知识*：三点 $(0,0)$、$(4,0)$、$(0,3)$ 能围成什么图形？写出它的周长。
  + *构造题*：在第二象限构造一个点，使它与 $(3, -2)$ 关于原点对称。写出它的坐标并验证。
  + *反例题*：判断"任意两个不同的点都能确定一条经过原点的直线"。给出反例。
  ...
  + 一样远。从原点到 $(2, 3)$：先向右 2、再向上 3，距离 $= sqrt(2^2 + 3^2) = sqrt(13)$；从原点到 $(3, 2)$：先向右 3、再向上 2，距离 $= sqrt(3^2 + 2^2) = sqrt(13)$。两者由勾股直觉得出*距离都是 $sqrt(13)$*。说明：横纵坐标互换不改变距离原点的远近，但*位置*完全不同（顺序很重要 vs 距离对称）。
  + 围成直角三角形（以原点为直角顶点）。三边长：$|(0,0) -> (4,0)| = 4$（水平）；$|(0,0) -> (0,3)| = 3$（竖直）；斜边 $|(4,0) -> (0,3)| = sqrt(4^2 + 3^2) = 5$（勾股）。周长 $= 3 + 4 + 5 = 12$（跨距离 + 三角形）。
  + 关于原点对称的点 $= (-3, 2)$（横纵坐标都变号）。横坐标 $-3 < 0$、纵坐标 $2 > 0$ → 在*第二象限* ✓。验证：$(-3, 2)$ 与 $(3, -2)$ 关于原点对称（中点 $= ((−3+3)/2, (2−2)/2) = (0, 0) = $ 原点 ✓）。
  + 反例：取 $(1, 2)$ 与 $(2, 4)$，它们与原点*共线*（都在直线 $y = 2 x$ 上）；但取 $(1, 2)$ 与 $(2, 1)$ 时，过它们两点的直线斜率 $= (1 - 2) / (2 - 1) = -1$，截距 $= 3$，写为 $y = -x + 3$，*不过原点*。所以并非"任意两个点"都决定一条过原点的直线——只有当两点和原点共线时才行（共线是过原点的必要条件）。
  ]
]
