...
#discovery[
  *先用旧工具试试。* 我们可以量角——用量角器量出三角形的三个角，再相加。量第一个三角形：$62° + 55° + 63° = 180°$。量第二个：$90° + 30° + 60° = 180°$。又是 $180°$！猜测：所有三角形内角和都是 $180°$。

  试着推下去——

  再量十个、一百个，都是 $180°$。但量角器有误差——画一个极扁的三角形，量角器读数可能是 $1°、1°、179°$，加起来 $= 181°$，是量角器精度问题，还是这个三角形例外？样本再多，也无法排除"下一个三角形可能例外"的可能；而且不知道"为什么是 $180°$ 而不是 $181°$ 或 $179°$"，只是碰巧量出来的。

  *这里卡住了。* 量角积累的是"样本都符合"，但无法说明"为什么必须如此"——我们需要一种用已知规律推出结论、对所有三角形一次性成立的推理方法。
...
#pitfall[
  *高频错误*
  ...
  视觉上看不到那个被压扁出来的“接近平角”的钝角，于是误以为总和缩水了。
...
#mastery[
  ...
  *挑战*
  ...
  + *[反直觉]* 有人说："边越长的三角形，内角和也越大。" 真的吗？请用边长 $1$ 的等边三角形与边长 $100$ 的等边三角形比较内角和说明。
  + *[跨知识点]* 三角形 $A B C$ 中 $angle A : angle B : angle C = 2 : 3 : 4$，它是锐角、直角还是钝角三角形？再用 #secref("tool:pf08-parallel-transversal") 的"过一顶点作对边的平行线 + 同位角 / 内错角"，说明为什么三个内角能拼成 $180°$。
  + *[构造题 / 反例]* 是否存在三个内角分别为 $89°, 89°, 2°$ 的三角形？给出验证；并用它说明"两锐角之和大于 $90°$ 就一定是钝角三角形"为什么是错的。
  ...
  + 不对——内角和恒为 $180°$，与边长无关。等边三角形不论边长 $1$ 或 $100$，三个角都是 $60°$，和都是 $180°$。"边越长内角和越大"的直觉来自把"边长 / 周长"和"角度"两套独立度量混在一起（内角和 $= 180°$ 与尺度无关）。
  + 总份数 $2 + 3 + 4 = 9$，每份 $180° div 9 = 20°$，所以 $angle A = 40°, angle B = 60°, angle C = 80°$，最大角 $80° < 90°$，是锐角三角形。证明拼成 $180°$：过 $A$ 作 $B C$ 的平行线 $ell$，由 #secref("tool:pf08-parallel-transversal") 的内错角 / 同位角，$angle B$ 等于 $ell$ 上 $A$ 一侧的角，$angle C$ 等于另一侧的角；这两个角加上 $angle A$ 正好拼成 $A$ 处的平角 $180°$，于是 $angle A + angle B + angle C = 180°$。
  + 存在：$89° + 89° + 2° = 180°$ ✓。它的两个 $89°$ 都还是锐角（差 $1°$ 才到直角），第三角 $2°$ 也是锐角，最大角 $89° < 90°$，所以是*锐角三角形*。这说明"两锐角和 $> 90°$ → 钝角三角形"是错的：本例两锐角和 $178°$ 极大，但根本没有钝角。判断钝角只能看单独有没有 $> 90°$ 的角，不能用两小角之和倒推（角分类标准 = 看最大角）。
  ]
]
