...
#crisis[
  ...
  平面图形只能描述“摊开的那一层”——牛奶盒、易拉罐、足球的形状，到底该怎么说？
]
...
#mastery[
  ...
  *挑战*
  ...
  + *反直觉*：球只有 1 个面（曲面），那它有几个顶点？几条棱？为什么？
  + *跨知识*：把 2 个相同的正方体并排粘在一起（共享一整个面），新立体的面、棱、顶点数是不是简单"翻倍"（即 12 / 24 / 16）？正确数应是多少？
  + *构造题*：构造一个具有 5 个面、6 个顶点、9 条棱的立体，并验证欧拉关系 $V - E + F = 2$。
  + *反例题*：判断"凡是有 6 个面的立体都是长方体"。给出反例。
  ...
  + 球：1 个面（曲面），*0 个顶点*、*0 条棱*。原因：顶点和棱都是"两个面交出"的产物——球只有 1 个面，没有"两面相交"，所以没有棱也没有顶点（面 / 棱 / 顶点的相互关系）。
  + *不能简单翻倍*。两个棱长为 1 的立方体共享一整个面后，合体的结果其实就是一个 $1 times 1 times 2$ 的长方体——粘合处的两面藏到了内部，原本围在粘合面四周、各占两个立方体的相邻侧面也*共面合并*成 1 个长方形。所以面数 $= 6$（不是 10），棱数 $= 12$（不是 20），顶点 $= 8$（不是 16）——和一个长方体的计数完全一样。验证欧拉：$V - E + F = 8 - 12 + 6 = 2$ ✓。要点：合体后，"两面之间的接缝"如果两侧已经共面，就不再是一条几何棱（共面相邻 → 合并为一面）。
  + 例：三棱柱（底是三角形，等高抬起）。$V = 6$（上下两三角形顶点）、$E = 9$（上 3 + 下 3 + 立 3）、$F = 5$（上下两个三角面 + 3 个矩形侧面）。$V - E + F = 6 - 9 + 5 = 2$ ✓。
  + 反例：三棱柱有 5 个面（不是 6），先排除。改用六棱锥也只有 7 面，也不是 6。真正的 6 面非长方体的例子：一个*斜四棱柱*（底是平行四边形，但不是矩形；侧面是四个平行四边形而非矩形）——它有 6 个面但不是长方体。又如*三棱台*（上下两个相似三角形 + 3 个梯形侧面）= 5 面，也排除。结论：长方体的判定还要求"6 个面都是矩形 + 相对面平行 + 相邻面互相垂直"（6 面 ≠ 长方体）。
  ]
]
