...
#blueprint[
  *查漏表*
  ...
  按“上一节里没答对的题”定位到补缺点，再决定继续前进或先补稳。
  ...
  #table(
    ...
    stroke: 0.4pt,
    [*卡在哪一类题*], [*常见原因*], [*忘了可以回到*],
    [小数与分数互化乱], [没把它们看成"同一个量的不同写法"], [#secref("pf04-fraction-equivalence")、#secref("pf05-decimals-conversion")],
    [混合运算顺序出错], [忘了先乘除、后加减，或不会用运算律凑整], [#secref("pf02-order-laws")],
    [带单位的题立刻乱], [没先把单位统一再算], [#secref("pf05-units-measurement")],
    [分数比较不稳], [不会把分母统一就直接比大小], [#secref("pf04-fraction-comparison")、#secref("pf04-fraction-equivalence")],
...
#blueprint[
  *共通桥梁即将登场的新旧对照*
  ...
  共通桥梁（Part II）里的 6 座桥梁会反复用到公共基础里学过的知识：
  ...
  换句话说：本章不是要教新公式，而是让你在进入桥梁前，能随时把这些前面学过的知识调出来。
]
...
#blueprint[
  *可以继续前进的最终信号*
  ...
  - 对照“查漏表”，能立刻指向哪一节补缺，而不是重刷整章。
  - 在诊断题里，即便做错，也能在看答案后说清“我当时该使用哪条前面学过的知识”。

  如果这三条你都能做到，你已经准备好进入共通共通桥梁。
]
