...
#side-hack[
  *Estimate-then-Solve 框架*——以后做应用题，养成"先估再算再比较"的习惯：

  示例题：超市买 $4$ 样东西，单价分别约 $19.5$、$8.7$、$3.2$、$25.4$ 元，总价是多少？

  + *先估*：不动笔精确计算，凭直觉给一个数量级的估计。把你的估计值写下来。\
    （这里取整：$20 + 9 + 3 + 25 = 57$ 元。）
  + *再算*：用学过的方法精算。\
    （$19.5 + 8.7 + 3.2 + 25.4 = 56.8$ 元。）
  + *比较*：精算结果和估计相差多少？如果差超过两倍，可能哪一步出了错？\
    （$56.8$ 与 $57$ 仅差 $0.2$ 元，量级一致 ✓。）

  ☞ 这个习惯能提前抓住计算错误——估算给你一道"防线"。任何应用题都可以套这三步：先估 → 再算 → 比较。
]
