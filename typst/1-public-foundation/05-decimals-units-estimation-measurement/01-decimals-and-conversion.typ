...
#discovery[
  *先用旧工具试试。* 我们已经有分数——表示"不足 1"的量。要把 $0.37$ 这个小数转成分数，先猜：好像是 $37/100$，但"为什么分母是 100"？凭感觉还是有规则？试试把 $0.5$ 按同样猜法写成 $5/10$——再检验：$5/10 = 1/2$，和 $0.5$ 确实相等。猜对了。
  ...
  *这里卡住了。* "分数和小数之间到底如何对应"没有系统规则——我们需要从整数的位值结构出发，弄清楚小数的每一位究竟代表哪个分母的几分之几。
