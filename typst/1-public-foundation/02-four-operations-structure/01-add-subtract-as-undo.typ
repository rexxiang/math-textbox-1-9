...
#discovery[
  *先用旧工具试试。* 我们只有加法——把数量合并起来。面对"现在有 63 支，刚放进去 25 支，原来有多少"，加法派上场：试 $63 + 25 = 88$？不对，又多了一批。换个方向：$25 + ? = 63$，凑一凑，$25 + 30 = 55$ 还不够，$25 + 38 = 63$，对了。

  试着推下去——

  靠"凑"找到了 38。但下次遇到"现在有 2847，放进去 594，原来有多少"，还要从 594 开始一个一个试加吗？何况若题目稍一变化——"放进了两批，一批 594，一批 213，原来有多少"——要凑两次，凑哪个先凑哪个全靠运气，没有任何系统规则。

  *这里卡住了。* 加法只能往前合并，没有"往回走找原量"的配套操作——我们需要一个能系统撤销加法、从结果倒推原始数量的新运算。
