...
#discovery[
  ...
  *小雯*：对。那"5 个 4"是不是一个*整体*？你先算 $3 + 4$，就把 $4 times 5$ 这个整体拆开，拿出一个 $4$ 跟 $3$ 合到一起了。
  ...
  *阿明*：那分配律 $6 times (9 + 1) = 6 times 9 + 6 times 1$ 呢？这个改写好像把括号也拆开了。
  ...
  #side-hack[
    *先停一停*——别急着看小雯的回答。想一想：分配律这样改写，原来式子里的"打包"关系变了没有？把你的判断写下来，再往下读。
  ]

  *小雯*：你看，左边是 6 组 $9 + 1$，右边是 6 组 9 加 6 组 1——*组数没变*，每组的内部份额也没变，结构是一致的。这种改写不破坏意思，所以是合法的。严格地说，等式左右表示同一个值；这条安全改写就叫分配律。
...
#side-hack[
  *为什么这个工具有效？*
  ...
  分配律不是"规定好的规则"，而是面积模型给出的事实：把一个长方形横向切成两块，两块面积之和必然等于整体——切开前后，方块总数没变。所以 $a times (b + c)$ 与 $a times b + a times c$ 必然相等。规则只是把这件几何事实写成代数式。
]
