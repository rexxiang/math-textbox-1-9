...
全书分成 6 个大块：公共基础 *pf*、共通桥梁 *cb*、代数 *al*、几何 *ge*、函数 *fn*、数据 *dt*。四支分支工具各自独立生长，但它们*都站在 pf + cb 这 15 + 10 个地基工具上*。本节把“哪些地基工具被*几个*分支同时调用”排成一张可复查的表。
...
#blueprint[
  *四支分支的“核心新工具”与它依赖的旧工具*
  ...
  你会发现：每一支的“标志性发明”其实是“旧工具按新方式组合后产生的一张新图”。*没有哪一支是凭空出现的*。
]

#side-hack[
  *一句话总览*：全书 6 大模块像一张地基网。公共基础 *pf* 与共通桥梁 *cb* 是地下管网；四支分支是地上建筑。卡在任何一层，先问自己：*下一层还稳吗？*
]
