...
#discovery[
  *先用旧工具试试。* 分式方程的直觉做法：两边同乘公分母，把分母消掉，变成整式方程。解 $3/x = 1/4$：两边乘 $4x$，得 $12 = x$——代回验一验，$3/12 = 1/4$，正确，没有任何问题。再解 $2/(x-1) = (x+1)/(x-1) + 1$：两边乘 $(x-1)$，得 $2 = (x+1) + (x-1)$，即 $2 = 2x$，$x = 1$。

  试着推下去——

  把 $x = 1$ 代回原式左边：$2/(1-1) = 2/0$——分母为零，原式根本没有意义；右边 $(1+1)/(1-1) + 1 = 2/0 + 1$，同样无意义。可是推导过程每一步都"没算错"：等式变形合法，整式方程也正确解出了 $x = 1$。

  *这里卡住了。* 去分母这一操作*扩大了方程的定义域*：整式方程允许 $x = 1$，原分式方程不允许——多出来的 $x = 1$ 既不是算错，也不是真解。我们需要一条规则来*甄别并剔除这种"操作引入的假解"*。
