...
#pitfall[
  *高频错误*
  ...
  Rania 解 $1/(x - 2) + 3 = (x - 1)/(x - 2)$ 时这样写：

  #block(inset: (left: 12pt))[
    第 1 行：两边同乘 $x - 2$，得 $1 + 3(x - 2) = x - 1$。

    第 2 行：$3 x - 5 = x - 1$，所以 $2 x = 4$，$x = 2$。

    第 3 行：答案是 $x = 2$。
  ]

  *错处*：第 3 行忘了验原分母。把 $x = 2$ 代入原方程的分母，$x - 2 = 0$，原式无意义。$x = 2$ 是去分母带来的*增根*，必须舍去；所以原方程*无解*。
