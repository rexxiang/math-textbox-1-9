...
#blueprint[
  *逐条勾选——能不查书独立做到的才算 ✓*
  ...
  - ☐ 能把任意一个单项式写出*系数 + 字母部分 + 次数*三件套，并指出 $-3 x^2 y$ 的系数是 $-3$ 而不是 $3$。（#secref("al01-monomials-coefficient-degree")）
  ...
  - ☐ 能化简任意分式：先因式分解分子分母，再约去公因式；通分时主动写最简公分母。（#secref("al03-rational-expression-structure")、#secref("al03-common-denominator-operations")）
  ...
  - ☐ 在一元二次方程上能用 $Delta = b^2 - 4 a c$ 判断根的数目，并用韦达定理验证答案。（#secref("al05-quadratic-solving")、#secref("al05-vieta")）
  - ☐ 能解一元一次不等式，并清楚“两边乘 / 除以负数要变号”不是规则而是*距离的几何意义*。（#secref("al05-linear-inequalities")）

  #line(length: 100%, stroke: 0.3pt + luma(200))

  *打勾不到 $9$ 条*：先停下，回到打不上勾的那一节重读。 \
  *打勾 $9$ 到 $11$ 条*：可以做 #secref("meta:al-self-test") 的混合自测，再针对失分点回看。 \
  *$12$ 条全部打勾*：恭喜，代数分支正式过关——可以转到任一其他分支，或继续读 #secref("meta:al-repair-routes") 把易错点再过一遍。
]
