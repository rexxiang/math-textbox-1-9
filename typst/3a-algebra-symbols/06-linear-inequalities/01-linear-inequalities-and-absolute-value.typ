...
#discovery[
  方程有等式性质：两边同乘同一个数，等式仍成立。把这条原封不动搬到不等式：$3 < 5$ 两边同乘 $2$，得 $6 < 10$，没问题。现在解 $100 - 3x >= 10$，移项得 $-3x >= -90$，两边除以 $-3$——按"等式那样"直接写 $x >= 30$。

  试着推下去——

  取 $x = 40$（满足 $x >= 30$）代回：$100 - 3 times 40 = 100 - 120 = -20$，而 $-20 >= 10$ 是假的。$x = 40$ 不是解，但"直接除 $-3$"给出的结论说它是。反过来取 $x = 10$（不满足 $x >= 30$），$100 - 30 = 70 >= 10$ ✓——它反而是正确的解。

  把等式性质原封不动搬到不等式，在"除以负数"这一步会得出方向错误的解集；同一道题代入几个具体数验证，立刻发现解集和不等号方向对不上。
  ...
  *把等式性质搬过来，只有一条会坏*
  ...
  #table(
    ...
    [两边*同乘*或*同除*一个*正*数（必非零）], [方向*不变*], [$3 < 5 => 6 < 10$],
    [两边*同乘*或*同除*一个*非零负*数], [*方向反转*（$<$ 变 $>$，$<=$ 变 $>=$）], [$3 < 5 => - 3 > - 5$],
  )

  ☞ *两边乘 $0$* 不是上面任何一种：两边都变成 $0$，原来的"大小"信息被压平成 $0 = 0$，*不等关系完全失效*。所以"乘除"那两条规则隐含的前提是：乘 / 除的那个数*不为零*。
...
#side-hack[
  *完整示例*——除以负数时为什么要翻向
  ...
  + *看题*：解不等式 $-3x + 5 < 17$。
  + *选工具*：先像方程一样做同向变形；一旦两边同除以负数，不等号方向要反过来。
  + *动手*：
    $ -3x + 5 &< 17 \
      -3x &< 12 \
      x &> -4. $
  + *回看*：取 $x = 0$，原式 $5 < 17$ 成立，且 $0 > -4$；取 $x = -5$，原式 $20 < 17$ 不成立，且 $-5 > -4$ 也不成立。翻向后和原题一致。
]
...
#pitfall[
  ...
  *错处*：第 2 行。除以负数时，不等号方向必须反转。这个错误还可以用代入自查：$x = 0$ 满足 $x >= -3$，但代回原式得到 $0 >= 9$，明显不成立。
  ...
  *自查句*：只要乘或除的是负数，先把不等号翻过去，再继续算；算完用边界点和一个测试点代回检查。

  另外两种常见走错：

  ✗ 把 $|x| < r$ 拆成 $x < r$ → ✓ 正确是 $-r < x < r$，少写一边就丢掉了负数解。

  ✗ 把 $|x| > r$ 写成 $x > -r$ 或 $x > r$ → ✓ 正确是 $x < -r$ 或 $x > r$，两段用“或”连起来。

  ✗ 解不等式组时只看其中一条 → ✓ 每条解集都画到同一条数轴上，最后取重叠部分。
]
...
#mastery[
  ...
  *挑战*
  ...
  + *[反直觉]*：判断："不等式两边同时乘 $x$，不等号方向不变。" 给反例（$x$ 可正可负可为零）并指出*只有乘正数才保号*——这就是 #secref("tool:al04-linear-one-variable") 中"等式两边同乘任意非零数"在不等式里失效的情形。

  + *[构造题]*：构造一个一元一次不等式组，要求它的解集恰好是空集（无解）。给一例并简述为什么"两个区间不相交"。

  + *[找 Bug]*：小华解 $-2 (x - 3) >= 4$，写："$-2 x + 6 >= 4$，$-2 x >= -2$，$x >= 1$。" 她错在哪一步？正确解集是？
  ...
  *挑战*
  ...
  + 错。反例 $2 > 1$，两边乘 $x = -1$：$-2$ 与 $-1$，应有 $-2 < -1$（不等号反向）；$x = 0$ 时两边都 $0$，不等号变等号——没法保留。规律：乘正数保号、乘负数反号、乘零失效。这与 #secref("tool:al04-linear-one-variable") 等式中"两边乘任意非零数仍保等"形成对比。
  + 一例：$cases(x > 5, x < 3)$。两个解集 $(5, infinity)$ 与 $(-infinity, 3)$ 不相交，故方程组解集为 $emptyset$（空集）。原因：两个条件互相矛盾，没有 $x$ 同时满足。
  + 错在最后一步。$-2 x >= -2$ 两边乘 $-1/2$（负数）应*反向*：$x <= 1$，不是 $x >= 1$。她忘了"乘除负数翻不等号"。正确解集 $x <= 1$。
  ]
]
