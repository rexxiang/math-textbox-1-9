...
#discovery[
  *先用旧工具试试。* 方程有等式性质：两边同乘同一个数，等式仍成立。把这条原封不动搬到不等式：$3 < 5$ 两边同乘 $2$，得 $6 < 10$，没问题。现在解 $100 - 3x >= 10$，移项得 $-3x >= -90$，两边除以 $-3$——按"等式那样"直接写 $x >= 30$。
  ...
  *这里卡住了。* 把等式性质原封不动搬到不等式，在"除以负数"这一步会得出方向错误的解集。不等式需要一条*专门说明乘除负数时不等号方向如何变化*的规则，这正是等式和不等式之间最关键的差异。
...
#pitfall[
  *高频错误*
  ...
  解方程时乘除等号没动作，习惯就把这套照搬到不等式上——但不等号会因负数翻面。

  ✗ 把 $|x| < r$ 拆成 $x < r$
  → ✓ 正确是 $- r < x < r$；少一半就丢掉了 $x$ 可以是负数那一段。
  根源在于：平时把 $|x|$ 当成“去掉负号”，于是写不等式时也只剩“正的那半”。
