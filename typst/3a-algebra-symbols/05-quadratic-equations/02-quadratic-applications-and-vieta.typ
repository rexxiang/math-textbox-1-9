...
#mastery[
  *基础*
  ...
  + 写出 $a x^2 + b x + c = 0$（$a != 0$）的韦达和、韦达积公式。
  + $x^2 - 3 x + 2 = 0$ 中，$a = ?, b = ?, c = ?$
  + 直接写出 $x^2 - 9 x + 14 = 0$ 的两根之和。
  ...
  + 方程 $x^2 - (m + 2) x + 2 m = 0$ 的两根 $x_1, x_2$ 满足 $x_1^2 + x_2^2 = 8$，求 $m$。
    ...
    #side-hack[
      *思路骨架*（卡住了再看下面的步骤）：
      ...
      + 第 1 步：题目给的是两根的*对称式*（$x_1^2 + x_2^2$）——能直接求根吗？不能，因 $m$ 未定；但凡涉及对称式都该想哪条工具？
      + 第 2 步：用一个恒等式把 $x_1^2 + x_2^2$ 转成 $x_1 + x_2$ 与 $x_1 x_2$ 的多项式（这恒等式你自己能写出吗？）。
      + 第 3 步：和与积分别由韦达定理写成关于 $m$ 的式子，代入；得到关于 $m$ 的方程并解。
      + 第 4 步：对每一个候选 $m$ 验 $Delta >= 0$（韦达定理需"实根存在"），不满足的舍去。
...
#history-note[
  *朱莉娅·罗宾逊（Julia Robinson，$1919$–$1985$）*——美国数学家。她毕生研究的核心问题之一是希尔伯特 $1900$ 年提出的第十问题：是否存在一种通用算法，可以判定任意整系数多项式方程（即"丢番图方程"）有没有整数解？$1970$ 年，俄国数学家马蒂亚谢维奇（Matiyasevich）在她和戴维斯、普特南奠定的基础上完成了最后一步，证明*这样的通用算法不存在*。她也是首位当选美国国家科学院院士的女性数学家，并担任美国数学会首位女会长。这一节的"用一元二次方程解整数应用题"，已经触到丢番图方程的边界。
]
