...
#crisis[
  ...
  有没有一种方法，对*所有*形如 $a x^2 + b x + c = 0$ 的方程都奏效——不靠运气，不用猜因式？
]

#tryit[
  + 解 $x^2 + x - 12 = 0$（先凭直觉给一对解，再核对——找两个数和为 $1$、积为 $-12$，符号容易错）。
]
...
#discovery[
  ...
  $7$ 和 $-1$：积 $= -7$ ✓，和 $= 6 != 1$ ✗；$1$ 和 $-7$：积 $= -7$ ✓，和 $= -6 != 1$ ✗；再试非整数 $p$、$q$……这样试下去没有终点，因为无理数的候选项无穷多。

  因式分解依赖于"碰巧能找到整数 $p$、$q$"；对根不是有理数的方程完全失效，只能靠猜，猜不中时无法判断"是没找到"还是"根本不存在这样的整数"。再多试几组系数，照样卡在同一个坎上。
...
#pitfall[
  ...
  #block(inset: (left: 12pt))[
    *题目*：用配方法解 $x^2 + 6 x - 7 = 0$。

    第 1 行：移项 $x^2 + 6 x = 7$。

    第 2 行：补"一次项系数 $6$ 的平方"：$+ 6^2 = + 36$。

    第 3 行：$x^2 + 6 x + 36 = 7 + 36 = 43$，写成 $(x + 6)^2 = 43$。

    第 4 行：$x = -6 plus.minus sqrt(43)$。
  ]

  ✎ 代回 $x = -6 + sqrt(43) approx 0.56$，原式 $approx 0.31 + 3.35 - 7 = -3.34$，不等于 $0$。哪里出错？

  #block(width: 100%, fill: rgb("#FFFDE7"), radius: 2pt, inset: 8pt)[
    *卡在哪里*：第 2 行配方时漏了"除以 $2$"。要把 $x^2 + 6 x$ 配成 $(x + h)^2 = x^2 + 2 h x + h^2$，对照 $2 h = 6$ 得 $h = 3$，所以补的是 $h^2 = 3^2 = 9$，*不是 $6^2 = 36$*。Amara 直接把一次项系数原封不动平方，跳过了"先除以 $2$"这一步。

    *正确做法*：
    $ x^2 + 6 x + 9 = 7 + 9 quad => quad (x + 3)^2 = 16 quad => quad x + 3 = plus.minus 4 quad => quad x = 1 "或" -7 $

    检验 $x = 1$：$1 + 6 - 7 = 0$ ✓；$x = -7$：$49 - 42 - 7 = 0$ ✓。
  ...
  ✗ 开平方时漏写 $plus.minus$ → ✓ $(x - 2)^2 = 9 => x - 2 = plus.minus 3$。
...
#side-hack[
  *双解法对比*：解方程 $x^2 - 5 x + 6 = 0$。

  + *方法一（因式分解）*：$x^2 - 5 x + 6 = (x - 2)(x - 3) = 0$，故 $x = 2$ 或 $x = 3$。
  + *方法二（求根公式）*：$x = (5 plus.minus sqrt(25 - 24)) / 2 = (5 plus.minus 1) / 2$，故 $x = 3$ 或 $x = 2$。

  两种方法都得 $x = 2, 3$。比较：因式分解一眼看出"$2 + 3 = 5, 2 dot 3 = 6$"就能写出，几乎不用计算；公式法机械可靠，但要算判别式和开方。

  ☞ 实战中：常系数能拼出整数对时优先因式分解；系数复杂或拼不出时直接用公式法。
]
...
#mastery[
  ...
  *挑战*
  ...
  + *[反直觉]*：方程 $x^2 = x$ 是否有两个不同的解？写出所有解并解释为什么不能直接"两边除以 $x$"。
  + *[组合]*：利用韦达定理：若二次方程 $x^2 - 5x + k = 0$ 的两根之差为 $1$，求 $k$。
  + *[构造题]*：写出一个二次方程，使它有两个根，且两根都是正整数，两根之积为 $12$。给出两个不同的例子。
...
#answer-cut[
  ...
  *挑战*
  ...
  + *[反直觉]*：$x^2 = x => x^2 - x = 0 => x(x-1) = 0$，解为 $x = 0$ 或 $x = 1$（两个不同解）。不能两边除以 $x$，因为 $x$ 可能等于 $0$——除以 $0$ 无意义，会丢掉 $x = 0$ 这个解（因式分解 + 防止除以零）。
  + 设两根为 $r_1, r_2$，韦达：$r_1 + r_2 = 5$，$r_1 r_2 = k$。两根差 $|r_1 - r_2| = 1$：$(r_1 - r_2)^2 = (r_1 + r_2)^2 - 4 r_1 r_2 = 25 - 4k = 1$，故 $k = 6$（韦达定理 + 配方）。
  + *[构造题]*：两正整数根积为 $12$，可以是 $(1, 12), (2, 6), (3, 4)$。例一：根 $2, 6$，方程 $(x-2)(x-6) = x^2 - 8x + 12 = 0$；例二：根 $3, 4$，方程 $(x-3)(x-4) = x^2 - 7x + 12 = 0$（因式分解逆用 + 构造）。
]
]
