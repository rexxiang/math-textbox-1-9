...
#discovery[
  *先用旧工具试试。* 解 $x^2 - 5x + 6 = 0$：找两个数和为 $5$、积为 $6$——$2$ 和 $3$ 刚好；因式分解为 $(x-2)(x-3) = 0$，解是 $x = 2$ 或 $x = 3$，干净利落。再试 $x^2 + x - 7 = 0$：找两个整数和为 $1$、积为 $-7$——

  试着推下去——

  $7$ 和 $-1$：积 $= -7$ ✓，和 $= 6 != 1$ ✗；$1$ 和 $-7$：积 $= -7$ ✓，和 $= -6 != 1$ ✗；再试非整数 $p$、$q$……这样试下去没有终点，因为无理数的候选项无穷多。

  *这里卡住了。* 因式分解依赖于"碰巧能找到整数 $p$、$q$"；对根不是有理数的方程完全失效，只能靠猜，猜不中时无法判断"是没找到"还是"根本不存在这样的整数"。我们需要一种*不靠运气、对所有 $a x^2 + b x + c = 0$ 都奏效*的通用方法。
...
#pitfall[
  *高频错误*
  ...
  把“没解”和“等于 $0$”画了等号——其实“无解”指的是这条方程在实数里压根找不到 $x$。
]
