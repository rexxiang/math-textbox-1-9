#import "../../lib/theme-v2.typ": crisis, discovery, blueprint, mastery, history-note, side-hack, vocab, tryit, pitfall, secref, answer-cut
#import "../../lib/diagram-packages.typ": cetz

=== 建模：设元、译句与“答得合理” <tool:al04-modeling-word-problems>

#vocab[设元 introducing an unknown][等量关系 equality relation][合理性检验 sanity check][建模 modeling]

#crisis[
  方程真正的威力不在“做给定的式子”，而在*把一段文字变成式子*。可很多同学到应用题就卡住——不知道“设谁”、不知道“怎么列”、更不知道算出的数“合不合理”。

  举个典型例子：

  > 一根长 $48$ 厘米的铁丝围成一个长方形，*长是宽的 $2$ 倍*。长和宽各是多少？

  可以设宽、也可以设长、甚至可以设周长的一半——*同一道题有多种设元*，但只要设得好，剩下两件事就变成机械活：*把原文再读一遍、用数学符号翻译一遍*。然后最后一步，*把答案代回情境里问一句：这样合理吗？*

  这一节不是要教你记住“行程问题 / 工程问题 / 配比问题”怎么做——那样你每遇到新情境都得重新学。真正需要学的是*同一条建模流程*，它对任何情境都适用。
]

#tryit[
  + 甲、乙两地相距 $300$ 千米，一辆汽车从甲出发以 $80$ 千米/时前进，另一辆摩托车从乙同时出发相向，速度 $70$ 千米/时。几小时后相遇？
]

#discovery[
  *建模三步：设元 → 译句 → 检验*

  + *设元*：选一个（或两个）*你能用式子表示其他量*的未知量，记作 $x$（或 $x, y$）。
  + *译句*：把原文里“是”“等于”“比…多”“比…少”“一共”“打折后付”等关键词，*一句一句*翻译成等式。题里有几句独立的等量关系，就能列出几条方程。
  + *检验合理性*：把解代入原文情境——答案的*量纲、正负、数量级*都必须讲得通。不合理的解（例如“宽 $= -5$ 厘米”）要*舍去*，而不是留着。

  #figure(
    cetz.canvas(length: 0.45cm, {
      import cetz.draw: *

      let bw = 8
      let bh = 4
      let gap = 3

      // Box 1: 设元
      rect((0, 0), (bw, bh), fill: rgb("#BBDEFB"), stroke: 1.2pt + rgb("#1565C0"), radius: 4pt)
      content((bw / 2, bh - 1), text(size: 8pt, weight: "bold", fill: rgb("#0D47A1"))[设元])
      content((bw / 2, 1.2), text(size: 6pt, fill: rgb("#1565C0"))[选未知量 $x$])

      // Arrow 1→2
      line((bw + 0.3, bh / 2), (bw + gap - 0.3, bh / 2), stroke: 1pt + rgb("#757575"),
           mark: (end: "stealth", fill: rgb("#757575"), scale: 0.5))

      // Box 2: 译句
      let x2 = bw + gap
      rect((x2, 0), (x2 + bw, bh), fill: rgb("#FFE0B2"), stroke: 1.2pt + rgb("#E65100"), radius: 4pt)
      content((x2 + bw / 2, bh - 1), text(size: 8pt, weight: "bold", fill: rgb("#E65100"))[译句])
      content((x2 + bw / 2, 1.2), text(size: 6pt, fill: rgb("#E65100"))[文字 → 等式])

      // Arrow 2→3
      line((x2 + bw + 0.3, bh / 2), (x2 + bw + gap - 0.3, bh / 2), stroke: 1pt + rgb("#757575"),
           mark: (end: "stealth", fill: rgb("#757575"), scale: 0.5))

      // Box 3: 检验
      let x3 = x2 + bw + gap
      rect((x3, 0), (x3 + bw, bh), fill: rgb("#C8E6C9"), stroke: 1.2pt + rgb("#2E7D32"), radius: 4pt)
      content((x3 + bw / 2, bh - 1), text(size: 8pt, weight: "bold", fill: rgb("#2E7D32"))[检验])
      content((x3 + bw / 2, 1.2), text(size: 6pt, fill: rgb("#2E7D32"))[合理？✓])
    }),
    caption: [建模三步流程：设元、译句、检验合理性],
  )

  *译句的关键是“等量关系”*

  每一句“…等于…”“…等于…的几倍”“…共…”都是一条等量关系。若原文有

  - $n$ 条独立的等量关系 + $m$ 个未知量，且 $n = m$，一般能确定唯一解；
  - $n < m$ → 方程不够，通常需要再读一遍原文，看是否漏掉隐藏条件（单位换算、整数约束、非负约束等）；
  - $n > m$ → 可能有冗余方程（多余条件），做完后要检查是否相容。

  *合理性检验不是“把数代回去验算”*

  验算只能兜住“算错”。合理性检验还要问：

  - 是不是正数？是不是整数？是不是在原文给出的范围内？
  - 单位对不对？“宽 $= 15$ 厘米”，但题目说长不超过 $20$ 厘米——那“长 $= 2 times 15 = 30$ 厘米”就超限了，必须回头检查设元或等量关系。

  *演示：长方形围丝*

  > 铁丝长 $48$ 厘米，长是宽的 $2$ 倍，求长和宽。

  *设元*：设宽 $x$ 厘米，则长 $2 x$ 厘米。\
  *译句*：“围成长方形”→ 周长 $= 2 (长 + 宽) = 2 (2 x + x) = 6 x$；“铁丝长 $48$”→ 等量关系 $6 x = 48$。\
  *解*：$x = 8$，故宽 $= 8$ 厘米、长 $= 16$ 厘米。\
  *检验*：两者都是正值，单位一致；$2 times (8 + 16) = 48$ ✓。

  *几种常见情境都在同一条流程上*

  #table(
    columns: (1fr, 1.3fr, 1.4fr),
    inset: 6pt,
    stroke: 0.4pt,
    [*情境关键词*], [*常用设元*], [*等量关系模板*],
    [行程（相向 / 追及）], [设时间 $t$ 或速度 $v$], [路程 $=$ 速度 $times$ 时间；两段路程之和 $=$ 总路程],
    [工程（甲乙合做）], [设时间 $d$ 或工作量 $u, v$], [工作量之和 $=$ 全部工作；约定“全部 $= 1$”],
    [比例 / 配比], [设份数 $k$ 或比中的一量], [各部分按比例写，加起来 $=$ 总量],
    [价格 / 利润], [设原价 $x$ 或数量 $n$], [折后价 / 成本 / 利润 $=$ 约束条件],
  )

  #figure(
    cetz.canvas(length: 0.45cm, {
      import cetz.draw: *

      let total-w = 30
      let bar-h = 3

      // Total distance bar (outline)
      rect((0, 0), (total-w, bar-h), stroke: 1.2pt + rgb("#424242"), radius: 2pt)

      // Car distance from left: 80t portion
      // 80/(80+70) * 30 = 16
      let car-w = 16
      rect((0, 0), (car-w, bar-h), fill: rgb("#BBDEFB"), stroke: 1pt + rgb("#1565C0"), radius: 2pt)
      content((car-w / 2, bar-h / 2), text(size: 7pt, weight: "bold", fill: rgb("#0D47A1"))[汽车 $80 t$])

      // Motorcycle distance from right: 70t portion
      let moto-w = total-w - car-w
      rect((car-w, 0), (total-w, bar-h), fill: rgb("#FFCDD2"), stroke: 1pt + rgb("#C62828"), radius: 2pt)
      content((car-w + moto-w / 2, bar-h / 2), text(size: 7pt, weight: "bold", fill: rgb("#B71C1C"))[摩托 $70 t$])

      // Meeting point marker
      line((car-w, -0.5), (car-w, bar-h + 0.5), stroke: 1.5pt + rgb("#2E7D32"))
      content((car-w, bar-h + 1.5), text(size: 7pt, fill: rgb("#2E7D32"), weight: "bold")[相遇])

      // Arrow left side
      line((0.5, -1.5), (car-w - 0.5, -1.5), stroke: 0.8pt + rgb("#1565C0"),
           mark: (end: "stealth", fill: rgb("#1565C0"), scale: 0.4))
      // Arrow right side
      line((total-w - 0.5, -1.5), (car-w + 0.5, -1.5), stroke: 0.8pt + rgb("#C62828"),
           mark: (end: "stealth", fill: rgb("#C62828"), scale: 0.4))

      // Total label
      content((total-w / 2, -3), text(size: 7pt, weight: "bold")[全程 $300$ 千米])
      line((0, -2.5), (total-w, -2.5), stroke: 0.6pt + rgb("#757575"),
           mark: (start: "|", end: "|", scale: 0.4))
    }),
    caption: [行程问题的线段图：汽车与摩托车相向而行，两段路程之和 $= 300$ 千米],
  )

  表里列的是不同情境的“变奏”——但拆开看都是*设元 + 译句 + 检验*同一条流程。不要为每种情境记死一套步骤。
]

#side-hack[
  建模时*最怕的是贪快*：看到题就想“这是行程题”“这是工程题”，直接套公式。稳妥做法是：*读两遍原文 → 用最笨的方法设元（挑你最熟悉的一个量） → 一句一句翻译等量关系*。流程慢 $30$ 秒，换来的是对多种题型都稳的方法。
]

#tryit[
  不要直接看下面的方法总结。先自己按*设元 → 译句 → 检验*做：

  + 一件工作，甲独做 $15$ 天完成、乙独做 $10$ 天完成。两人合做几天完成？
]

#history-note[
  用方程为实际问题建模的传统极为悠久。“鸡兔同笼”最早见于《孙子算经》（约公元 3—5 世纪），“百钱买百鸡”出自南北朝张丘建《张丘建算经》（约公元 5 世纪）。在西方，古埃及莱因德纸草书（约公元前 1650 年）的“aha 问题”同样是先给情境、再找等量关系。这些跨越千年和大洲的题目有一个共同结构：先选定一个未知量，再*按原文句子的意思*写出等量关系——把情境剥到这层，方法就统一了。
]

#blueprint[
  *工具一：建模三步流程*

  + *设元*：选能用来表达“其他相关量”的一个（或两个）未知量 $x$（或 $x, y$）。
  + *译句*：把原文中每一句描述“是 / 等于 / 多 / 少 / 共”的句子翻译成*一条等式*。
  + *检验合理性*：代回情境——看正负、单位、量纲、数量级；不合理的解舍去。

  *工具二：等量关系翻译模板*

  - 行程：*路程 = 速度 $times$ 时间*；两段路程之和 $=$ 总路程。
  - 工程：把整件工作约定为 $1$；每人的“日工作量”写成分数。
  - 比例：先写“每份是多少”，再用份数之和 $=$ 总量。
  - 价格：折后价 $=$ 原价 $times$ 折扣；利润 $=$ 售价 $-$ 成本。

  *工具三：合理性检验的三把尺子*

  + *量纲*：答案单位是不是符合题意？
  + *符号*：有没有出现“负宽”“负时间”之类不合理的解？
  + *范围*：答案是否落在题目暗含的范围内（整数、有限区间）？

  *例 1*（行程相遇）：设 $t$ 小时后相遇。$80 t + 70 t = 300 => t = 2$。检验：$t = 2$ 正值、合理。相遇时两车共行 $300$ 千米 ✓。

  *例 2*（工程合做）：设合做 $d$ 天完成。$d / 15 + d / 10 = 1 => (2 d + 3 d) / 30 = 1 => d = 6$。检验：$d = 6 > 0$，代回分数和等于 $1$ ✓。

  *例 3*（配比）：设浓缩液 $k$ 毫升，水 $4 k$ 毫升。$k + 4 k = 500 => k = 100$。浓缩液 $100$ 毫升、水 $400$ 毫升；总量 $500$ 毫升 ✓。
]

#blueprint[
  *设未知量五步路*

  拿到任意一道应用题，按这条流程走，几乎不会迷路：

  + 找问题问的*核心量*——题目最后那个问号问的是谁，那一个量就是设元的最佳候选。
  + 用一个字母（一般是 $x$）*命名*这个核心量；写一行“设〔某某〕为 $x$”。
  + 把题目里其它已知量*用 $x$ 表达*——把每一句话里的“某某”尽量改写成 $x$ 的代数式。
  + 找*等量关系*——读句子，盯住“是 / 等于 / 加起来等于 / 比……多 / 比……少 / 共”这些词，每一处都对应一条方程候选。
  + *列方程并解*；最后一步把解*回代到原文情境*验证（量纲、正负、范围）。

  下表把这五步对应到“铁丝围长方形”这个最小例子：

  #table(
    columns: (auto, 1fr),
    inset: 6pt,
    stroke: 0.4pt,
    [*步骤*], [*铁丝长 $48$、长是宽的 $2$ 倍*],
    [1. 核心量], [题目问长和宽——选*宽*作为基准（也可以选长，效果等价）。],
    [2. 命名], [设宽 $= x$ 厘米。],
    [3. 用 $x$ 表达其它量], [长 $= 2 x$ 厘米；周长 $= 2(长 + 宽) = 6 x$ 厘米。],
    [4. 等量关系], [“铁丝长 $48$”→ 周长 $= 48$，即 $6 x = 48$。],
    [5. 列方程、解、回代], [$x = 8$；宽 $= 8$、长 $= 16$；代回 $2(8 + 16) = 48$ ✓。],
  )

  这五步对前面表格里所有“情境关键词”（行程、工程、配比、价格）都通用——情境换了，*流程不换*。
]

#blueprint[
  *建模三步口诀*——以后看应用题，按这三步走：

  + *设元*：把"未知量"用字母（如 $x$）命名；写下"$x$ 表示什么"。
  + *译句*：把题目里的中文关系翻译成等式；每个等式都问"等号两边数量是否一致"。
  + *检验*：解出 $x$ 后回到题目情境，问"这个答案在现实里讲得通吗"（人数不能是分数 / 钱不能为负 / 时间不能超界）。

  ☞ 这三步贯穿全书所有应用题——从分数应用题、比例问题、到方程组、二次方程、不等式。
]

#pitfall[
  *高频错误*

  ✗ 没有显式“设元”就开始列式
  → ✓ 先写一句“设〔某某〕为 $x$”，再动笔——否则你会在自己脑子里偷偷换了变量。

  ✗ 把“比…多”翻译成乘法
  → ✓ “$A$ 比 $B$ 多 $5$”是加法：$A = B + 5$，不是 $A = 5 B$。读一遍原文再翻译。
  中文里“多”和“倍”都跟“数量增大”有关，听起来像，写式子就容易混。

  ✗ 得到解就直接写答案，不做合理性检验
  → ✓ 尤其是宽 / 时间 / 数量这类*必须非负*的量，发现 $x < 0$ 要舍去；有时候要再考虑“是不是设元选反了”。

  ✗ 把“答案”当作“未知量”
  → ✓ 题目问“长和宽”，你设了“宽为 $x$”，算完要*把长也写出来*；不要只回答 $x$。
]

#mastery[
  *基础*

  + 把"比 $x$ 多 $7$"翻译成代数式。
  + 把"$y$ 的 $3$ 倍少 $2$"翻译成代数式。
  + 设小明今年 $a$ 岁，他 $5$ 年后多大？$3$ 年前多大？
  + 一支笔 $3$ 元，买 $n$ 支共多少元？

  + 一个数的 $3$ 倍比它的 $2$ 倍多 $14$，求这个数。
  + 某班共 $45$ 人，男生人数比女生多 $5$。男生、女生各几人？
  + 一个两位数，十位上的数字是个位的 $2$ 倍；交换位置后新数比原数小 $36$。求原数。

  *应用*

  + 甲有苹果 $120$ 个、乙有 $60$ 个。甲每天卖 $15$ 个、乙每天卖 $5$ 个。几天后两人的苹果数相等？
  + 某商品成本 $20$ 元。为保证利润不低于 $15%$，售价最少定多少？（请用方程/不等式思维，按本节建模流程写完设元、等量关系与检验。）
  + Priya比小李大 $4$ 岁。$5$ 年后Priya的年龄是Luca的 $3/2$ 倍。两人今年各几岁？

  *挑战*

  + 一列火车匀速穿过一座 $500$ 米长的桥用 $30$ 秒；以同样速度穿过一条 $1300$ 米长的隧道用 $70$ 秒。求火车速度与车身长。（提示：设车长 $L$、速度 $v$，列方程组。）

    #side-hack[
      *思路骨架*（卡住了再看下面的步骤）：

      + 第 1 步：弄清"穿过"——车头进入桥头到车尾离开桥尾，*总路程是什么*？这道题需要几个未知量？
      + 第 2 步：把两个时间事件分别写成一条方程（每条形如"路程 = 速度 $times$ 时间"），得到一个二元一次方程组。
      + 第 3 步：两式相减消去其中一个未知量，先解另一个；再回代解第一个。
      + 第 4 步：合理性检验——速度与车长都应为正；把米/秒换成千米/时。

      下面是对应的题目，按上面的思路试试：
    ]
  + 某顺水行船的速度比静水速度大 $3$ 千米/时；顺水 $30$ 千米与逆水 $18$ 千米用时相等。求船在静水中的速度。
  + 一个班有 $a$ 名男生、$b$ 名女生，男生和女生分别平均考了 $80$ 与 $76$ 分，全班平均 $78$ 分。用比例法（无需具体数字）说明男生人数与女生人数的关系。

  + *[反直觉]*：甲、乙两人各自独立做一件工作，甲需 $4$ 天，乙需 $6$ 天。两人合作，是不是 $4 + 6 = 10$ 天完成？正确的合作天数是多少？
  + *[组合]*：100 克浓度 $30 %$ 的食盐水与 $x$ 克浓度 $10 %$ 的食盐水混合，要使混合后浓度为 $20 %$。列方程求 $x$，并验证答案是否使两个浓度恰好"平衡"。
  + *[构造题]*：自己编写一道"行程问题"应用题（包含速度、时间、距离三个量，且需要解一个一元一次方程），并给出完整解题过程。

#answer-cut[

  *基础*
  + $x + 7$。
  + $3 y - 2$。
  + $5$ 年后 $a + 5$ 岁；$3$ 年前 $a - 3$ 岁。
  + $3 n$ 元。
  + 设这个数 $x$：$3 x = 2 x + 14 => x = 14$。（设元 + 移项）
  + 设女生 $x$：$x + (x + 5) = 45 => x = 20$。男生 $25$、女生 $20$。（设元 + 译句）
  + 设个位 $x$，十位 $2 x$：$10 dot 2 x + x - (10 x + 2 x) = 36$？不对，方向反了。原数为 $10 dot 2 x + x = 21 x$，新数 $10 x + 2 x = 12 x$，题目说“新数比原数小 $36$”：$21 x - 12 x = 36 => x = 4$。原数 $84$。检验：新数 $48 = 84 - 36$ ✓。（设元 + 译句 + 检验）
  *应用*
  + 设 $d$ 天后相等：$120 - 15 d = 60 - 5 d => 60 = 10 d => d = 6$ 天。
  + 设售价 $x$ 元：$x >= 20 times 1.15 = 23$。最少 $23$ 元。（设元 + 等量关系 + 检验：非负、单位合理。）
  + 设小李今年 $x$ 岁、Priya $x + 4$ 岁。$x + 4 + 5 = 3 / 2 dot (x + 5) => x + 9 = 3 x / 2 + 15 / 2 => x / 2 = 3 / 2 => x = 3$。小李 $3$ 岁，Priya $7$ 岁。

  *挑战*
  + $(L + 500) / v = 30$，$(L + 1300) / v = 70$；相减：$800 / v = 40 => v = 20$ 米/秒 $= 72$ 千米/时；$L = 100$ 米。
  + 设静水速 $v$：$30 / (v + 3) = 18 / (v - 3)$，交叉相乘 $30 (v - 3) = 18 (v + 3)$，$12 v = 144$，$v = 12$ 千米/时。检验 $v plus.minus 3 > 0$ ✓。
  + 全班平均 $(80 a + 76 b) / (a + b) = 78 => 80 a + 76 b = 78 (a + b) => 2 a = 2 b => a = b$，即男生、女生人数相等。
  + 反直觉：不是 $10$ 天，而是更短。每天甲做 $frac(1,4)$，乙做 $frac(1,6)$，合作每天做 $frac(1,4) + frac(1,6) = frac(5,12)$。合作天数 $= frac(12,5) = 2.4$ 天（工作效率叠加，不是时间叠加——效率模板 + 分数加法）。
  + 食盐守恒：$100 times 30 % + x times 10 % = (100 + x) times 20 %$，即 $30 + 0.1x = 20 + 0.2x$，$10 = 0.1x$，$x = 100$。验证：$(30 + 10)/(200) = 40/200 = 20 %$ ✓（溶液配比问题 + 方程建模）。
  + *[构造题]*：例：小明骑车上学，以 $12$ 千米/小时从家出发，走了 $15$ 分钟后发现书忘了。他立刻原路返回，以 $18$ 千米/小时跑步取书。问：返回家需要几分钟？解：已走 $12 times frac(1,4) = 3$ 千米，以 $18$ 千米/小时返回，需 $frac(3,18) times 60 = 10$ 分钟（行程模板 + 方程建模 + 构造）。
]
]

#history-note[
  *玛格丽特·汉密尔顿（Margaret Hamilton，$1936$–）*——美国数学家与软件工程师。她在 MIT 仪器实验室（Instrumentation Laboratory，今 Draper 实验室）工作期间，主持编写了阿波罗登月计划制导计算机的飞行程序。她正是"软件工程"（software engineering）这个词的首创者。把一道登月任务变成可以执行的代码，她的方法是：先把"飞船在哪里、要去哪里、燃料还剩多少"等条件抽象成变量，再用方程描述变量之间的关系，最后把方程翻译成程序——这与你这节做应用题的流程完全一致。$2016$ 年她获得美国总统自由勋章。
]
