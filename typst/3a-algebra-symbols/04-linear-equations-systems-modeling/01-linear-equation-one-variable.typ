...
#pitfall[
  ...
  ✎ 代回原式 $2(2) - (2 + 3) = 4 - 5 = -1$，不等于 $5$。哪里出错了？

  #block(width: 100%, fill: rgb("#FFFDE7"), radius: 2pt, inset: 8pt)[
    *卡在哪里*：第 1 行去括号时，Arjun 把 $-(x + 3)$ 写成 $-x + 3$。负号要*分别作用到括号里每一项*：$-(x + 3) = -x - 3$，里面的 $+3$ 也要变号。

    *正确做法*：
    $ 2 x - (x + 3) = 5 quad => quad 2 x - x - 3 = 5 quad => quad x - 3 = 5 quad => quad x = 8 $

    检验：$2 dot 8 - (8 + 3) = 16 - 11 = 5$ ✓
  ...
  ✗ 去分母时漏乘独立的常数 $1$ → ✓ 每一项都要乘以公分母，包括独立的常数 $1$ 也要乘成公分母大小。

  ✗ 移项时不变号 → ✓ 从 $3 x - 7 = 2 x + 5$ 到 $3 x - 2 x = 5 + 7$，挪过去的项变号。

  ✗ 得到 $0 = 0$ 就写"$x = 0$" → ✓ $0 = 0$ 是恒等式，原方程有无穷多解，不是"$x = 0$"这个具体值。
]
