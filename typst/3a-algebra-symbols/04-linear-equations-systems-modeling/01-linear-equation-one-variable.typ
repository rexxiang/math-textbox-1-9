...
#side-hack[
  *两条路对比：移项简写 vs 等式两边操作*

  #table(
    columns: (auto, 1fr, 1fr),
    inset: 6pt,
    stroke: 0.4pt,
    [*情境*], [*方法 A：移项简写*], [*方法 B：等式两边操作*],
    [何时更快], [方程无分母、两边都含 $x$（如 $5 x - 3 = 2 x + 9$）：直接把含 $x$ 的项移左、常数移右，一行写出 $3 x = 12$], [必须去分母时写"两边同乘公分母"，能防止漏乘某项（如 $frac(x - 1, 2) - frac(x + 3, 6) = 1$ 每一项都要乘 $6$，包括常数 $1$）],
    [典型陷阱], [从 $3 x - 7 = 5$ 移项忘翻号，写成 $3 x = 5 - 7$——挪过去的项必须同时变号], ["两边同除 $x$"在 $x$ 可能为 $0$ 时非法；应先移项化为 $a x = c$，再除以系数 $a$],
    [建议先用], [熟练后日常演算用移项简写，节省步骤；口诀：挪过去就变号], [去分母和"两边乘负数"时明写等式操作，初学阶段也方便逐步追溯依据],
  )

  *经验法则*：无分母时移项一步最快；含分母时先明写"两边同乘公分母"（这步不能跳），把分母清掉再移项。
]
