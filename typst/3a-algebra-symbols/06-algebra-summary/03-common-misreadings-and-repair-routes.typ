...
== 分支学完后的常见误读与查漏指引 <meta:al-repair-routes>
...
#side-hack[
  *本支与其它分支的接口*
  ...
  - 往几何分支去：*解未知边 / 角* 时列的是本支里的方程（#secref("al04-linear-one-variable") / #secref("al05-quadratic-solving")）；记得最后*合理性检验*——负长度舍掉。
  - 往函数分支去：$f(x) = c$ 相当于解方程；$f(x) > c$ 相当于解不等式。函数的“定义域”往往由分式与平方根的*非负 / 非零*条件决定，正是本支的工具。
