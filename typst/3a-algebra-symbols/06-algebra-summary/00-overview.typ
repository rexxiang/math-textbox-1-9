...
#blueprint[
  *难度*：★★★★☆ \
  *核心问题*：从桥梁章里“字母代替数”开始，一路走到一元二次方程和不等式——这条代数发明链到底长什么样？哪些工具是“同一件事的不同写法”？如果中途某处卡住，该回哪一章？ \
...
#side-hack[
  *本章路线*
  ...
  + §1 画出本支的发明树：从“字母 → 代数式 → 方程 → 因式分解 → 分式 → 二次与不等式”如何一路派生；每个工具都带着 `#secref` 回到本章。
  + §2 出几道*跨章综合题*——每题都显式注明它调用了哪些工具链，让你练习“在一道题里同时调用多章工具”。
  + §3 梳理分支学完之后最常见的*误读*与对应的*回补路径*——不是重刷全章，而是精确回到某一节。
