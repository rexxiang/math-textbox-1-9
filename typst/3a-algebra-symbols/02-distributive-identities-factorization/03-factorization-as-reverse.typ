...
#pitfall[
  ...
  #block(inset: (left: 12pt))[
    *题目*：把 $3 x^3 - 12 x$ 因式分解到底。

    第 1 行：两项都有公因式 $3 x$。

    第 2 行：$3 x^3 - 12 x = 3 x (x^2 - 4)$。$quad$ "提完了，结束。"
  ]

  ✎ 看上去工整，可"分解到底"四个字她漏算了。卡在哪？

  #block(width: 100%, fill: rgb("#FFFDE7"), radius: 2pt, inset: 8pt)[
    *卡在哪里*：括号里的 $x^2 - 4$ 还能继续分——它是平方差 $x^2 - 2^2 = (x + 2)(x - 2)$。Fatima 把"提完公因式"当成了"分解结束"，但因式分解的目标是*把每个能分的因式都分到不能再分*。

    *正确做法*：
    $ 3 x^3 - 12 x = 3 x (x^2 - 4) = 3 x (x + 2)(x - 2) $

    检查每个因式：$3 x$、$x + 2$、$x - 2$ 都是一次或常数，再无平方差或公因式可提，到此为止。
  ...
  ✗ 把 $x^2 + 9$ 当成 $(x + 3)^2$ → ✓ 实数范围内 $x^2 + 9$ 不能分解；完全平方必须匹配中间项 $2 dot x dot 3 = 6 x$。

  ✗ 中间项匹配时忽略符号 → ✓ $x^2 - 5 x + 6$ 中 $c = +6, b = -5$，找"同负"一对 $-2, -3$，得 $(x - 2)(x - 3)$。
]
...
#side-hack[
  *两条路对比：分组法 vs 套特殊公式*
  ...
  #table(
    ...
    inset: 6pt,
    stroke: 0.4pt,
    [*情境*], [*方法 A：分组法*], [*方法 B：套特殊公式*],
    [何时更快], [四项多项式两两有公因式（如 $a x + a y + b x + b y$）：分成 $a(x + y) + b(x + y)$，再提 $(x + y)$，两步到位], [两项形如 $A^2 - B^2$（如 $25 x^2 - 9$）或三项满足完全平方条件（如 $4 y^2 - 12 y + 9$，中间项 $= 2 times 2 y times 3$）：一眼认出直接套],
    [典型陷阱], [分组方式选错没有公因式可提：$x^3 + x^2 - x - 1$ 若分成 $(x^3 + x^2)$ 和 $-(x + 1)$ 可提出 $(x + 1)$；配对错了换一种分法重试], [误把 $a^2 + b^2$ 当平方差——实数范围内两平方之和无法因式分解；套完全平方前必须核对中间项是否恰好 $= 2 A B$],
    [建议先用], [先走三步流程（提公因式 → 识别公式 → 中间项匹配）；到"识别公式"这步无命中且项数为 $4$，再试分组], [一眼看出首末均为完全平方时直接套；若中间项验证不符合 $2 A B$，退到中间项匹配流程],
  )

  *经验法则*：两项式先看平方差；三项式先验完全平方的中间项是否 $= 2 A B$，不符再用中间项匹配；四项式优先试分组（两两配对提公因式）。
]
