...
#discovery[
  *先用旧工具试试。* 看到 $3x^3 - 12x$，想直接用"中间项匹配"——找两个数使和等于某系数、积等于常数项。但这是三次多项式，不是 $x^2 + b x + c$ 的标准二次三项式形式，中间项匹配的入口条件根本不满足。
  ...
  *这里卡住了。* 中间项匹配要求首项系数为 $1$ 的纯二次三项式；遇到系数不为 $1$ 或次数超过 $2$ 就失效，且题目里还有公因式 $3x$ 没有先提出来——这一步不做，任何后续工具都用不对。我们需要一套*有优先顺序的决策流程*。
