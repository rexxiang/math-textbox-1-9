...
#mastery[
  ...
  *挑战*
  ...
  + *[反直觉]* 小齐说："$(a + b)^2 = a^2 + b^2$，因为括号里是加法，平方就分别加。" 用 $a = 3, b = 4$ 计算左右两边证明她错了。然后回答：小齐为什么会这样想？她把*分配律* $a (b + c) = a b + a c$ 与*平方运算*混淆了——把"分配律对加法乘法成立"错套到"分配律对加法平方也成立"上。请指出漏掉的*交叉项*是哪个、为什么交叉项一定不能省（结合 $(a + b)^2 = (a + b)(a + b)$ 完整展开来看）。
