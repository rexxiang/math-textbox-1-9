...
#discovery[
  *先用旧工具试试。* 计算 $103 times 97$：老老实实列竖式，$103 times 97 = 103 times (100 - 3) = 10300 - 309 = 9991$。算出来了——但用了两次乘法加一次减法。遇到 $104 times 96$、$201 times 199$，每次都得重新算。
  ...
  *这里卡住了。* 逐题展开乘法行得通，但每次都在重复同一个模式（中间值 $plus.minus$ 偏差），却没有一条公式让我们*识别这种结构、直接跳过竖式*。我们需要把这类高频展开模式归纳成固定公式，正用和反用都能快速套用。
...
#pitfall[
  *高频错误*
  ...
  视觉上太对称、太顺眼，记忆里就把中间那项 $2 a b$ 偷偷省掉了。
...
#mastery[
  ...
  *挑战 ☞ 选做*
  ...
  + $a = 3, b = 4$：$(a + b)^2 = 7^2 = 49$；$a^2 + b^2 = 9 + 16 = 25$。差 $24 = 2 a b$，正是漏掉的交叉项。完全平方公式真正是 $(a + b)^2 = a^2 + 2 a b + b^2$。"分配律不能套到平方上"是因为平方是把整个量 $(a + b)$ 与自己相乘，按乘法分配律展开会出现 $a dot b + b dot a = 2 a b$ 这一交叉项；只有乘法对加法才有分配律，乘方对加法没有分配律。
