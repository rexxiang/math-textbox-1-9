...
#crisis[
  在 #secref("pf02-order-laws") 里，分配律一直只朝一个方向用：把 $3 (x + 4)$ *展开* 成 $3 x + 12$。这条工具看起来够用——直到你撞上这道方程：

  $ 6 x^2 + 9 x = 0 $

  你想"用展开把它化简"——可它已经全是单项相加，没有括号可拆，旧工具一上手就没活干。换个思路代入猜根：$x = 0$ 显然是一个解；那还有没有别的？$x = 1$ 代入是 $15$，$x = -1$ 是 $-3$，$x = -1.5$ 才得到 $0$。问题是你没法保证猜得到，更没法保证猜全。

  *卡点*：旧的分配律只会"左到右乘开"——这一题需要的恰恰是反过来——把 $6 x^2 + 9 x$ 重新写成"两个东西的*乘积*"，比如 $3 x dot ( ? + ? )$。一旦写成乘积，"乘积 $= 0$" 就立刻把方程拆成几条小方程：每个因子分别等于 $0$，哪个能解就解哪个。

  另外两道一样让旧工具失手的式子：

  - $5 a + 5 b$——能不能写得更短？
  - $x (y - 3) + 4 (y - 3)$——里面的 $(y - 3)$ 能不能像公因数一样拎出来？

  *分配律其实是一条双向公路*：左到右叫"乘开"，右到左叫"提公因式"。这一节就要把右到左这一头补齐——下面 discovery 把"反向看出公因式"在多个具体例子里走一遍。
]
...
#discovery[
  crisis 给的卡点是：要把 $6 x^2 + 9 x = 0$ 拆开来解，必须把左边*重新写成乘积*的样子，比如 $3 x dot ( ? + ? )$。先不急着记新名词——下面给你三个具体的式子，每给一个，自己试着"反向看出"那个被抽出来又放在外面的共同部分。

  *练习 1：*$6 x^2 + 9 x$

  #self-check[
    合上书或遮住下面，先自己写一个回答，再读后续。具体写：$6x^2$ 和 $9x$ 这两块，里面有什么*共同的乘数*可以"抽出来再放到括号外面"？把它写出来，并把改写后的式子写完整，再往下读。

    ☞ 写完再继续：
    - ✓ 如果你写出了 $3 x (2 x + 3)$，方向完全对——你抽出了 $3 x$ 这块共同部分。
    - ✓ 如果只看出 $3 (2x^2 + 3x)$ 或 $x (6x + 9)$，方向也对，只是没抽到底；下一步会教怎么"抽干净"。
    - ✗ 如果你抽出 $6x$ 或 $9x$，那两项里只有一项含得了它——抽出来的必须是*每一项里都有*的那块。
  ]

  *练习 2：*$10 a b + 15 a$

  #self-check[
    用同样的方法：找出每一项里都有的*那块乘数*，抽到外面，把剩下的写进括号。再往下读。

    ☞ 写完再继续：
    - ✓ 答案 $5 a (2 b + 3)$。系数取 $10, 15$ 的最大公因数 $5$；字母看哪个是两项都有的——是 $a$，$b$ 只在第一项有，不能抽。
    - ✗ 如果抽成 $5 a b (2 + 3 b^(-1))$ 这种，第二项就被迫"补 $b$"，破坏了"每项都有"的规则。
  ]

  *练习 3：*$x^3 - x^2$

  #self-check[
    再来一道，这次只看字母——$x^3$ 与 $x^2$ 里都有什么共同部分？把它抽出来。

    ☞ 写完再继续：
    - ✓ 答案 $x^2 (x - 1)$。共同部分取*相同字母里的最低次幂*——$x^3$ 含 $x^2$、$x^2$ 含 $x^2$，最低次就是 $x^2$。
    - ✗ 如果抽 $x$ 得 $x (x^2 - x)$，方向对但没抽到底，括号里还能继续抽 $x$。
  ]

  *给共同的动作起个名字*

  你刚才在三道题里反复做的同一件事——*把每项都有的那块乘数拎到括号外，括号里只剩各项除以它后的结果*——就叫*提公因式*。被拎出去的那块叫*公因式*；它正是分配律 $a(b+c) = a b + a c$ 反向读出来的样子：
  ...
  *把"抽到底"的规则写出来*
  ...
  回头看你做的三道练习：要让公因式*抽干净*，规则就是——*系数取最大公因数，相同字母取最低次幂*；括号里填的是原式每一项除以公因式后的结果。写完后可以验证：把它乘回去，能得到原式，就算提对了。
]
...
#history-note[
  ...
  ☞ 这条历史线索后续在： 韦达 $1591$ 年大量使用分配律展开通用代数式，为今天的多项式运算奠基；代数分支完整展开见 #secref("al02-distributive-rewrite")。
]
...
#side-hack[
  *完整示例*——同一个式子，展开和收回
  ...
  + *看题*：一边是 $3x(2x + 5)$，另一边是 $6x^2 + 15x$；要看它们怎样互相改写。
  + *选工具*：分配律可以正向展开，也可以反向提公因式。要去括号时展开；要看共同结构、为后面约分或解方程做准备时收回。
  + *动手*：展开：
    $3x(2x + 5) = 3x times 2x + 3x times 5 = 6x^2 + 15x$。
    反向：$6x^2$ 和 $15x$ 都含 $3x$，所以
    $6x^2 + 15x = 3x(2x + 5)$。
  + *回看*：展开后没有括号，便于合并同类项；提公因式后看见"同乘 $3x$"，便于把结构看清。
]
...
#mastery[
  ...
  *应用*
  ...
  + 同一题做“双向选择”：计算 $48 times 27 + 52 times 27$ 时，先判断该展开还是提公因式；再把你的结果反向乘开检验。
  ...
  *应用*
  ...
  + 应该提公因式 $27$，因为两项都有同一个因子：$48 times 27 + 52 times 27 = 27(48 + 52) = 27 times 100 = 2700$。反向乘开检验：$27(48 + 52) = 27 times 48 + 27 times 52$，回到原式，说明提取方向正确。
