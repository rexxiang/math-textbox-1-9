...
#discovery[
  数轴用一个数就能定位一条直线上的点。换到操场——以大门为原点，规定向东为正，对同学说"树在 $5$"——这一个数够吗？

  "在 $5$"只说了向东 5 步，可操场是一片平面：偏北 3 步偏东 5 步的点，和偏南 2 步偏东 5 步的点，"向东分量 5"都满足，整条南北方向的线上的点都可以叫"$5$"。改成"距大门 5 步"画一个半径 5 的圆，圆上每个点都符合，更不唯一。再加上方向"偏东 30°"确实能锁定一点；但两处营救点 $(5, 30°)$ 与 $(8, 45°)$ 的中点该取在哪里？直接算 $((5+8)/2, (30°+45°)/2) = (6.5, 37.5°)$ 不是几何中点，正确算法要用三角函数。

  一个数不够；"距离 + 角度"虽够，可中点、平移这种最常用的运算复杂得离谱。能不能找一种描述方式，让平移和中点都只用加减就能算？
