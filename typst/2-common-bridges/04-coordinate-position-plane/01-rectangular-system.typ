...
#crisis[
  ...
  *为什么偏偏是两个数？这两个数凭什么就能唯一确定平面上任意一点？*
  ...
  在大海上遇险时报"距港口 $50$ 海里"，救援队找不到你——以港口为圆心、$50$ 海里为半径整圈圆都满足；多报一个"$50$ 海里 $+$ 偏北 $30 degree$"才锁住位置。但如果同时两艘船遇难、要找"中间点"一次救两人，把角度和距离直接平均却不是几何中点。
...
#discovery[
  *一个数不够*
  ...
  数轴用一个数定位直线上的点。换到操场就不行了。

  对同学说"树在 $5$"——这一个数*不够*：偏北 $3$、偏南 $2$、"向东分量 $5$"都满足。改成"距大门 $5$"也不行——整圈圆都满足。

  平面是二维的，需要*两个数*才能唯一锁定一点。而且这两个数必须独立——改变其中一个不会强迫另一个跟着变。

  *极坐标 vs 直角坐标*

  锁定一点至少两种方案：*极坐标*报 $(r, theta)$（距离 $+$ 角度）；*直角坐标*用两条垂直数轴上的 $(x, y)$。两套都唯一，差别在*下游运算*——求中点试一下：

  - 极坐标：$lr((frac(r_1 + r_2, 2), frac(theta_1 + theta_2, 2)))$ 不是几何中点，得先化为直角坐标、求完再化回去，三角函数全程上场。
  - 直角坐标：$lr((frac(x_1 + x_2, 2), frac(y_1 + y_2, 2)))$，坐标各自取平均，纯加减。

  导航与工程里极坐标合适，但日常"中点 / 平移 / 整体挪一格"这类动作在直角坐标里都只剩加减——这是直角坐标后来成为代数与几何共同语言的根本原因。

  *直角坐标的搭法*

  两条数轴正交于同一点：水平叫 $x$ 轴（右正左负），竖直叫 $y$ 轴（上正下负），交点叫*原点* $O$。点 $P$ 的坐标 $(x, y)$ 就是它在两个方向上偏离原点的有符号距离。改变 $x$ 只让点水平移动、$y$ 不变（两维独立）；$(3, 5)$ 与 $(5, 3)$ 是两个不同的点（顺序固定：先横后纵，像"第 $14$ 排第 $7$ 座"和"第 $7$ 排第 $14$ 座"是不同的座位）。
]
...
#history-note[
  ...
  ☞ 这条历史线索后续在： 笛卡儿坐标系让函数的图像第一次可以被精确绘制；正比例函数 $y = k x$ 的直线图是最早被系统研究的例子，见 #secref("fn04-direct-variation")。
]
