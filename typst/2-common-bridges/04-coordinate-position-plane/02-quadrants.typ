...
#tryit[
  ...
  + 不用任何区域名字，只看坐标的正负号，把下列各点分组——把你认为"落在同一块方向"的点放在一起：$(4, 3)$、$(-2, 5)$、$(-1, -6)$、$(7, -8)$、$(2, 1)$、$(-3, -4)$。你分出了几组？每组的正负号规律是什么？
]
...
#history-note[
  $1637$ 年笛卡儿（Descartes）建立坐标系后，紧接着就出现一个实用问题：当点散落在四个方向时，怎样让大家说"那一块"时指的是同一块？$18$ 世纪的莱布尼茨（Leibniz）和欧拉（Euler）在解析几何著作中逐渐统一了"从右上角开始、逆时针编号"这个惯例。这一约定一旦确立，"第二象限"立刻就锁定了 $x < 0, y > 0$ 的那一块——每块取了拉丁语 _quadrans_（"四分之一"）这个名字，便是今天的"象限"。
]
...
#pitfall[
  *高频错误*
  ...
  ✗ 顺时针编号
  → ✓ 四象限是*逆时针*编号，从右上开始。
  日常写字、读时钟都习惯顺时针，逆时针这个约定确实需要专门记一下。
...
#mastery[
  ...
  *挑战 ☞ 选做*
  ...
  + *[反直觉]* 判断："如果 $(x, y)$ 在第三象限，那么 $(y, x)$ 也一定在第三象限。" 对吗？再判断 $(x, y)$ 在第二象限时，$(y, x)$ 会落在哪个象限？给出推理。
  + *[跨知识点]* 点 $P(a, b)$ 在第二象限，把 $P$ 关于原点 $O$ 作中心对称（即 #secref("tool:cb04-plotting-transforming") 的 $(x, y) → (-x, -y)$），所得点 $P'$ 在第几象限？再关于 $x$ 轴对称一次，最终落在哪？过程中横纵符号怎样变化？
  + *[构造题 / 反例]* 找一个点 $Q$，使 $Q$ 与它关于 $y$ 轴的对称点恰好在同一象限。能找到吗？给反例并说明。再找一个点使它与关于 $x$ 轴的对称点在同一象限。能找到吗？
  ...
  + 第一句*对*：$(x, y)$ 在第三象限即 $x < 0, y < 0$，交换得 $(y, x)$ 仍是 $(<0, <0)$，还在第三象限。但 $(x, y)$ 在第二象限时（$x < 0, y > 0$），$(y, x) = (>0, <0)$，落在*第四象限*。第二象限交换坐标会跨到对角的第四象限——交换坐标的效果取决于两坐标的符号是否相同。
  + 关于原点 $(x, y) → (-x, -y)$（呼应 #secref("tool:cb04-plotting-transforming")）：$P(a, b)$ 在第二象限（$a < 0, b > 0$），所以 $P'(-a, -b)$ 满足 $-a > 0, -b < 0$，在*第四象限*。再关于 $x$ 轴：$(-a, -b) → (-a, b)$，得到 $(-a > 0, b > 0)$，落在*第一象限*。整个过程符号变化：$(-,+) → (+,-) → (+,+)$。
  + 关于 $y$ 轴对称：$(x, y) → (-x, y)$，横坐标变号；除非 $x = 0$（在 $y$ 轴上、不属任何象限），否则对称点的横坐标符号一定相反，必然落在另一象限。所以*找不到*第二种点。关于 $x$ 轴对称：$(x, y) → (x, -y)$，纵坐标变号，同样除非 $y = 0$ 否则纵坐标符号必反，落在另一象限——也找不到（轴对称必然跨象限，除非点在对称轴上）。
  ]
]

#side-hack[
  ☞ "该段在第二象限"、"沿 $y$ 轴对称后点落在第几象限"——函数、几何、数据分析里都会用象限编号快速定位。
]
