...
#mastery[
  ...
  *挑战 ☞ 选做*
  ...
  + *[解释]*：上一题甲、乙两车路程相等。请用 $s = v t$ 模板的*结构*解释为什么"速度翻倍、时间减半"会使路程不变；并思考：如果速度乘以 $k$、时间乘以 $1/k$（$k > 0$），路程一般如何变化？这条规律意味着 $v$ 和 $t$ 之间的什么关系（哪种比例）？
  ...
  + 由 $s = v t$，乘积 $v t$ 决定路程。把 $v$ 换成 $k v$、$t$ 换成 $t / k$，得 $s' = (k v) times (t / k) = v t = s$，路程不变。这就是"速度时间互补"——保持路程不变时，$v$ 与 $t$ 成*反比例*关系（$v t = $ 常数）。这正是反比例函数 $t = s / v$ 的初步理解：速度涨多少倍，时间就缩多少倍。
  ]
]
