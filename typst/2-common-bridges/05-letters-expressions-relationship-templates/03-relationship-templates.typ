...
#mastery[
  ...
  *挑战*
  ...
  + 甲车速度是乙车的 $2$ 倍，同时出发 $t$ 小时后，甲比乙多走了多少千米？多走的路程占乙车路程的几倍？（结果用代数式表示）
  + 圆的周长 $C = 2 pi r$，面积 $S = pi r^2$。用 $C$ 来表示 $S$（消去 $r$），得到什么公式？当 $C = 10 pi$ 时 $S$ 是多少？
  + 某水箱已有水 $20$ 升，以每分钟 $5$ 升的速度注水，写出 $t$ 分钟后水量 $V(t)$ 的代数式，并求几分钟后水量达到 $60$ 升。
  + *[构造题]*：自己设计一个"匀速运动"情景，给出速度 $v$、出发时已走路程 $s_0$，写出 $t$ 时刻总路程 $s(t)$ 的代数式，然后给同桌出一道"$s(t) = 100$ 时求 $t$"的题，并附答案。
...
#answer-cut[
  ...
  + 甲速 $= 2v$，乙速 $= v$；$t$ 小时后甲走 $2v t$，乙走 $v t$，多走 $v t$ 千米。多走量占乙的 $frac(v t, v t) = 1$ 倍（即恰好多走乙的路程的一倍）（匀速位移模板 $s = v t$）。
  + 由 $C = 2 pi r$ 得 $r = frac(C, 2 pi)$。代入面积：$S = pi r^2 = pi (frac(C, 2 pi))^2 = frac(C^2, 4 pi)$。当 $C = 10 pi$：$S = frac((10 pi)^2, 4 pi) = frac(100 pi^2, 4 pi) = 25 pi$（代数式变换 + 消元）。
  + $V(t) = 20 + 5t$（线性增长模板）。令 $20 + 5t = 60$，$5t = 40$，$t = 8$ 分钟（代数式列方程求解）。
  + *[构造题]*：例：小明骑车，初始已走 $10$ 千米，速度 $15$ 千米/小时，$s(t) = 10 + 15t$。出题：何时总路程达 $100$ 千米？答：$10 + 15t = 100 => t = 6$ 小时（匀速位移模板 + 构造 + 解方程预热）。
]
]

#side-hack[
  ☞ 代数几乎所有应用题都在套这几个模板；函数里 $y = k x$ 就是匀速运动模板 $s = v t$ 的图形化版本。
]

#history-note[
  *葛丽丝·霍珀（Grace Hopper，$1906$–$1992$）*——美国数学家、海军少将。她在耶鲁取得数学博士学位后，参与了早期电子计算机的设计，并在 $1950$ 年代提出"编译器"的概念：让人写贴近自然语言的"模板式"指令，再由机器自动翻译成数字代码。她主持开发的 COBOL 语言至今仍在运行全球大量银行系统。她常说的一句话是："最危险的一句话是'我们一直都是这么干的'"——这话也适用于数学：当你识别出一个反复出现的关系模板，就要敢于把它抽出来，命名、复用，而不是每次重新算。
]
