...
#crisis[
  上一节已经把"用字母代替数"这件事讲透了。可问题来了——同一个长方形，老师上午让你算周长（"$2 + 2 + 3 + 3$"，你一笔笔加），下午让你算面积（"$2 times 3$"，你重新想一遍乘法）；明天题目把数字一换（边长改成 $4.7$ 和 $9.3$），你又要从"$4.7 + 4.7 + 9.3 + 9.3$"开始一笔笔加。

  *痛点不在算不出来——而在"每一题都从零想起"。* 三个具体例子立刻让旧办法失手：

  - 题目说"长方形周长是 $20$、长是 $7$，求宽"——你要不会反过来想，根本不知道从 $20$ 怎么倒推到宽。每换一种"已知 / 未知"的搭配，整个思路都要重新搭。
  - 同事在电话里报"路程 $360$ 千米、时间 $4.5$ 小时，速度是多少"——脑子里没有现成模板，只能现场推导"距离怎么和时间挂钩"。
  - 把求面积这件事搬到三角形上——"$1/2 times$ 底 $times$ 高"和长方形的"长 $times$ 宽"是不是同一回事？没有写下统一格式之前，每个图形都像头一回见。

  *卡点*：上一节的字母让我们"能写"（$a$ 和 $b$ 代替具体数），可如果这些用字母写出来的关系*不固定下来、不命名、不挂在图形旁边*，每一道题、每一种已知未知组合，都要把同一条关系临时再推导一遍——慢，并且每一次都可能出错。

  这一节要做的，是把这些反复出现的"关系"打包成几张可以直接调用的*模板*，给它们贴上名字（"长方形周长公式"、"距离公式"……）。下面 discovery 把最常用的几张模板一次拼齐。
]
