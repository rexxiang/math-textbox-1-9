...
#discovery[
  *先用旧工具试试。* 遇到简单问题，算术硬算：Priya存了 $3$ 周零花钱加上家长给的 $10$ 元共 $46$ 元——先减 $10$ 得 $36$，再除以 $3$ 得 $12$，每周 $12$ 元。可以算出来。但如果题目换成"先花了存款总额的一半，再花了 $8$ 元，最后还剩 $14$ 元，原来有多少"？

  试着推下去——

  设原有 $M$ 元：还剩 $M/2 - 8 = 14$，所以 $M/2 = 22$，$M = 44$。凑出来了——但为了找"先做什么后做什么"，要在脑子里*倒着拆*好几步，每道新题都得重新想顺序。

  *这里卡住了。* 算术法只能从已知"一步步往后推"；一旦未知量出现在被减、被乘等中间环节里，就必须反向拆解，没有系统。我们需要一种能*直接把未知量当已知去写约束关系*的新工具。
...
#pitfall[
  *高频错误*
  ...
  口算时把等号当成“接下来是”这种连接词在用，于是写出来也照搬。
  ...
  根源在于：把 $0$ 默认成“没东西”，于是觉得“$x$ 等于零”就是“没解”。
]
...
#mastery[
  ...
  *挑战*
  ...
  + $x = x$ 对*任何 $x$ 都成立*——它是恒等式，从"求解"角度看*有无穷多个解*（每个数都满足）。$x + 1 = x$ 化简得 $1 = 0$，*恒不成立*——*无解*。要点：方程的解可能"无穷多"也可能"无"——*取决于化简后是否仍含 $x$ 且能找到使其成立的值*（解的个数三种情形）。
