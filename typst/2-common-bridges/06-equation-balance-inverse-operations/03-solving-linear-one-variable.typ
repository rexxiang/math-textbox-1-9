...
#mastery[
  ...
  *挑战 ☞ 选做*
  ...
  + *[找 Bug]*：余婧（Yu Jing）解方程 $-2 (x - 3) = 8$ 时这样写："两边除以 $-2$，得 $x - 3 = 8 div (-2) = -4$，所以 $x = -1$。" 检查她的解，找出错误并给出正确解法。
...
#answer-cut[
  ...
  + 余婧的计算 $8 div (-2) = -4$ 正确，但代回原方程检验：$-2(-1 - 3) = -2 times (-4) = 8$ ✓。其实 $x = -1$ 没错！但她写 "$x - 3 = -4$，所以 $x = -1$" 这一步推理*符号是对的*：$x = -4 + 3 = -1$。所以这道题答案正确。如果题目是 $-2(x - 3) = 8$ 改写为 $-2(x + 3) = 8$（正负括号易混淆），则两边除以 $-2$ 得 $x + 3 = -4$，$x = -7$。本题真正的"找 Bug"提醒：解完务必代回原方程验证，避免错把符号弄反——这正是反向操作和代入检验的双保险。
