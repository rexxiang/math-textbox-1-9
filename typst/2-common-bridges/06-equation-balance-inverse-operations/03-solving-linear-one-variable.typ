...
#tryit[
  + 解方程：$5 - 2 x = 11$（先写下你的第一直觉答案，再仔细解一次）。
  + 解方程：$3 (x - 2) = 12$。
]
  ...
  + 解方程 $frac(x+2, 3) - frac(x-1, 2) = 1$。（需要通分后移项合并。）
    ...
    #side-hack[
      *思路骨架*（卡住了再看下面的步骤）：
      ...
      + 第 1 步：找到两个分母 $3$ 和 $2$ 的最小公倍数 $6$，*两边同乘 $6$* 一次性去分母（注意右边的常数项 $1$ 也要乘 $6$）。
      + 第 2 步：去分母后展开括号，把 $x$ 项和常数项分别合并。
      + 第 3 步：移项——把含 $x$ 的项放一边、常数项放另一边，注意"过等号变号"。
      + 第 4 步：把 $x$ 的系数化为 $1$，求出解；再代回*原方程*验证两边相等。
