...
#crisis[
  $2 x + 3 = 11$ 这样的题，凑数也能猜出 $x = 4$。但下面这道呢？

  $ 7 (2 x - 3) + 5 = 4 (x + 8) - 3 $

  现在请你"凭感觉猜一个值代进去试试"——猜了 $x = 1$ 不对，猜 $x = 5$ 也不对，猜 $x = 10$ 还不对。猜数这条路，在系数稍微变复杂时立刻失灵。

  上一节 #secref("cb06-equality-properties") 给了我们四条合法变形——可它只告诉你"哪些动作不会破坏等式"，没告诉你*该按什么顺序动手*。把"加"先做、还是把"乘开括号"先做、还是先把含 $x$ 的项归到一边？随便挑一条都能继续往下走，不一定走得通。

  $x$ 在题里被一层一层裹着：先是 $(2 x - 3)$ 这层括号，再被 $7$ 一乘，再加 $5$；右边的 $x$ 也被 $4$ 乘、被 $+8$ 包。如果四条性质上来就乱用一气，常常化简到一半发现，含 $x$ 的项越解越多、根本收不拢。

  *卡点*：合法变形不止一条路，但每一步都可能让局面更乱或更整齐。我们需要一条*固定的"拆包顺序"*——任何题都按这一条流程走，不依赖灵感、不靠凑数，复杂系数下也不会卡住。这一节就要把"反向操作"按"从最外层往里拆"的顺序串起来，让解题动作变得机械可靠。
]
...
#side-hack[
  *完整示例*——解 $7(2x - 3) + 5 = 4(x + 8) - 3$

  + *看题*：两边都有括号和 $x$，要找让等式成立的那个数。
  + *选工具*：一元一次方程的标准路线是：拆括号 → 合并同类项 → 把含 $x$ 的项放一边 → 验证。
  + *动手*：
    $ 7(2x - 3) + 5 &= 4(x + 8) - 3 \
      14x - 21 + 5 &= 4x + 32 - 3 \
      14x - 16 &= 4x + 29 \
      10x &= 45 \
      x &= 9/2. $
  + *回看*：代回 $x = 9/2$，左边 $= 7(9 - 3) + 5 = 47$，右边 $= 4(9/2 + 8) - 3 = 47$，两边相等。
]
