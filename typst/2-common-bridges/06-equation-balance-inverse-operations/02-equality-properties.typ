...
#discovery[
  *先用旧工具试试。* 解 $5x - 7 = 3x + 11$，直觉说"先把含 $x$ 的移到一边，数字移到另一边"——但凭什么说这步合法？试另一顺序：先两边加 $7$，得 $5x = 3x + 18$，再两边减 $3x$，得 $2x = 18$，$x = 9$。换个顺序：先两边减 $3x$，得 $2x - 7 = 11$，再两边加 $7$，得 $2x = 18$，$x = 9$——同样对。

  试着推下去——

  可如果操作错误，比如只在左边加 $7$，得 $5x = 3x + 11$，再减 $3x$，得 $2x = 11$，$x = 5.5$——和正确答案不同。两个过程都"看起来在化简"，只有一个正确，却没有任何规则来区分"合法"和"非法"的操作。

  *这里卡住了。* 靠直觉猜顺序有时凑对、有时出错，而没有一条可以查阅的规则说明"对等式做哪些操作是合法的"。我们需要把"对两边同时做同样的事"明确写成四条可检验的性质。
...
#mastery[
  ...
  *挑战*
  ...
  + *[找 Bug]* 小白要解 $(x - 1) / 2 = (x + 3) / 5$，他第一步写："两边同乘 $10$，得到 $x - 1 = x + 3$。" 错在哪里？正确该怎样乘？
