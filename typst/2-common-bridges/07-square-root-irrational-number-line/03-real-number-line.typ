...
#crisis[
  上一节刚证完：$sqrt(2)$ 不是任何分数。可它*一定有*一个具体长度——画一个边长 $1$ 的正方形，沿对角线拉一根线，这根线就是它，伸手可量。

  现在把这根线段的一端按到数轴原点上、让它沿正方向躺平。线段的另一端落在数轴上的什么位置？

  - 它不在 $1$ 上（$1^2 = 1 < 2$）。
  - 它不在 $2$ 上（$2^2 = 4 > 2$）。
  - 它不在 $1.4$、不在 $1.5$、不在 $99/70$——上一节已经证过，*没有任何分数 $p / q$ 的平方等于 $2$*。

  问题来了：上一节我们刚说过有理数在数轴上"稠密"——任意两个有理数之间还能塞进无穷多个有理数。"稠密"听起来像"已经把数轴铺满了"。可现在这根 $sqrt(2)$ 长的线段的端点*实实在在地落在数轴上某个位置*，而那个位置上偏偏没有任何一个有理数。

  *"稠密"和"铺满"竟然不是一回事。* 旧的有理数集 $QQ$ 看起来挤得密不透风，可一旦从几何里拿来一段"边长 $1$ 的正方形对角线"，就立刻指出一个 $QQ$ 没法标记的位置。

  类似的窟窿不止一个：圆周长比 $pi$、$sqrt(3)$、$sqrt(5)$、$root(3, 2)$……每一个都对应数轴上一个具体位置，每一个都不是分数。

  数轴上这些"有具体位置、却没有分数能标的点"，到底是几个、几十个，还是更多？要不要给它们一个名字、配一个完整的数集？这一节把这件事补完——下面 discovery 先把 $sqrt(2)$ 的位置精确画出来，再说为什么必须扩出*实数*。
]
...
#history-note[
  $1872$ 年，德国数学家戴德金（Richard Dedekind）发表了《连续性与无理数》，用"戴德金分割"严格定义了实数的连续性。同年，康托尔（Georg Cantor）用柯西序列给出了另一种等价的构造。在此之前，数学家们凭直觉使用实数已有两千多年；但"数轴上没有缝"这一事实直到 $19$ 世纪才真正被证明。中学里我们先用直觉描述它：实数轴是一条没有"缝"的直线。
]
...
#self-check[
  在数轴上找出下列三个数对应的点，并按从小到大排好：$sqrt(2)$、$3/2$、$1.4$。哪两个最接近？这告诉你“数轴上每个点都对应一个实数、每个实数也对应一个点”这件事在哪里起作用？
]
