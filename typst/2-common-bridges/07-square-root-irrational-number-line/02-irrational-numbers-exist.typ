...
#side-hack[
  *重演毕达哥拉斯学派的发现：$sqrt(2)$ 不能写成分数*

  相传古希腊毕达哥拉斯学派的成员希帕索斯（Hippasus）发现了无理数，引起轩然大波。下面你来重演那个经典的反证思路（不要求完整背出来，跟着步骤走就行）：

  假设 $sqrt(2) = p/q$，其中 $p, q$ 是正整数，且 $p/q$ 已是最简分数（$p, q$ 没有公因数）。

  + 两边平方：$2 = p^2 / q^2$，即 $p^2 = 2 q^2$。这说明 $p^2$ 是偶数还是奇数？
  + 由第 1 步，能推出 $p$ 是偶数吗？（提示：奇数的平方是奇数。）
  + 若 $p = 2 k$（$k$ 为正整数），代入 $p^2 = 2 q^2$ 得 $4 k^2 = 2 q^2$，化简后 $q^2 = ?$。这说明 $q$ 是偶数还是奇数？
  + $p$ 和 $q$ 同时是偶数，与"最简分数"的假设矛盾了吗？

  #answer-cut[
    + $p^2 = 2 q^2$ 是偶数（等于 $2$ 乘以某数）。
    + 是：若 $p$ 是奇数则 $p^2$ 是奇数，与 $p^2$ 是偶数矛盾，所以 $p$ 必须是偶数。
    + $4 k^2 = 2 q^2 => q^2 = 2 k^2$，即 $q^2$ 是偶数，同理 $q$ 是偶数。
    + 矛盾！$p, q$ 都是偶数，说明 $p/q$ 有公因数 $2$，与假设最简矛盾。因此 $sqrt(2)$ 不能写成 $p/q$，即无理数。
  ]
]
...
#mastery[
  *基础*
  ...
  + 判断 $sqrt(49)$ 和 $sqrt(50)$ 哪个是有理数、哪个是无理数。
  + 无限小数 $0.101001000100001dots$（$0$ 的个数递增）是有理数还是无理数？
  ...
  *挑战 ☞ 选做*
  ...
  + $sqrt(49) = 7$ 是有理数（$49$ 是完全平方数）；$sqrt(50)$ 是无理数（$50$ 不是完全平方数，小数判别法）。
  + 无理数（小数不循环也不终止，不满足有理数的“有限或循环”特征）。
  + 反证法：假设 $sqrt(3) = p/q$ 既约。平方：$p^2 = 3q^2$。$3 | p^2$ 故 $3 | p$（$3$ 是质数），设 $p = 3k$，代回 $9k^2 = 3q^2$，$q^2 = 3k^2$，故 $3 | q$。$p, q$ 都被 $3$ 整除，与既约矛盾。
  + 边长 $= sqrt(5)$（开方是求面积的反向操作）。$5$ 不是完全平方数，所以 $sqrt(5)$ 是无理数。
  + 例如 $sqrt(2) / 2$ 或 $pi / 4$。
