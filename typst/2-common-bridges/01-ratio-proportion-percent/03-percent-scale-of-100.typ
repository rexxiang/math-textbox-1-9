...
#mastery[
  ...
  *挑战 ☞ 选做*
  ...
  + *[解释]*：上一题的结果出乎多数人意料："先加 $10\%$ 再减 $10\%$"竟然不是回到原点。请用代数式 $p times (1 + 0.1) times (1 - 0.1)$ 展开，解释为什么乘法增长 / 减少和加法增长 / 减少不一样——百分数的加减需要明确"基数是谁"。
  ...
  + $p times 1.1 times 0.9 = p times (1 - 0.01) = 0.99 p$，比 $p$ 少 $1\%$。原因：第一次"$+10\%$"的基数是 $p$，加的是 $0.1 p$；第二次"$-10\%$"的基数变成了 $1.1 p$，减的是 $0.11 p > 0.1 p$。两次百分数针对的基数不同，因此不抵消。一般地 $(1 + r)(1 - r) = 1 - r^2 < 1$（$r != 0$），永远比 $1$ 略小。这就是为什么*百分数的加减必须先看清基数*。
  ]
]
