...
#discovery[
  *先用旧工具试试。* 我们只有具体的份数。奶茶配方“茶 2 份、奶 5 份”——想问“哪个配方奶味更重”，就比绝对份数：配方 A 用了 5 份奶，配方 B 用了 8 份奶，B 奶更多。

  试着推下去——

  但绝对份数会随做的杯数变化。再让配方 A 多做几杯：A 做 $4$ 杯（茶 $8$ 份、奶 $20$ 份），B 仍做 $1$ 杯（茶 $3$ 份、奶 $8$ 份）。现在 A 的奶有 $20$ 份，B 的奶只有 $8$ 份——按绝对份数应该说"A 奶更多"，结论和上一轮反过来了。可两个配方本身完全没变，"奶味浓不浓"是配方自己的属性，凭"绝对份数"得出的结论却随杯数任意翻转。

  *这里卡住了。* 绝对份数随杯数变化，把“配方比例”和“做了多少杯”混在一起分不开——我们需要一种只锁住两个量之间的相互关系、不随总量变化的写法。
