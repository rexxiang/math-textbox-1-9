...
#crisis[
  这一条数轴现在装了什么？前两节装上了所有整数：$..., -3, -2, -1, 0, 1, 2, 3, ...$。整数和整数之间，按"每两个整数之间隔一格"的样子整整齐齐排着。

  可现实里大量出现的数*根本不是整数*——

  - 今天最高气温 $-3.5 degree$C：在数轴上指向 $-3$ 还是指向 $-4$？两个都不对，因为它落在两者之间。
  - 银行年利率 $-1/4 \%$（亏损）：要在 $0$ 和 $-1$ 之间一个细致的位置上。
  - 长方形对角线分一半：从 $-2$ 到 $+5$ 的中点是 $1.5$——就连"两整数之间的中点"这种最基础的几何运算，结果都常常不是整数。

  原本"数轴上只能放整数"这条隐含前提，到这里完全卡死：*整数装不下负的小数、装不下分数、装不下任意两点的中点*。每碰到一个非整数，都得说"在第 $-3$ 格和第 $-4$ 格之间偏右的某处"——既不精确，也不可重复。

  分数小学就学过、负号上一节也接好了。可分数有正有负、整数也有正有负——它们能不能住在同一根数轴上？$1/2$、$-3/4$、$0.7$ 各自的"那个点"在哪、由谁来定位？
]
