...
#discovery[
  ...
  想直接读出距离：$-7$ 的距离 $= -7$？但距离不能是负数——"负 7 步远"不是一段距离。那去掉负号：距离 $= 7$。好，这对了。再看乙（$-3$）：距离 $= 3$。现在问"谁欠更多"（谁距原点更远）：$7 > 3$，甲欠更多。但如果规则只是"去掉负号比大小"，遇到 $-7$ 和 $+5$，谁距原点更远？去掉负号：$7$ 和 $5$，甲更远——可 $+5$ 本来就是正数，距离本来就是 $5$，去不去负号结果一样，但负数这步"去掉负号"的操作凭什么对正数也适用？没有统一名字，每次都要分类讨论。
